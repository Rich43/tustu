...
/* loaded from: icepdf-core.jar:org/icepdf/core/pobjects/annotations/TextAnnotation.class */
public class TextAnnotation extends MarkupAnnotation {
    private static final Logger logger = Logger.getLogger(TextAnnotation.class.toString());
    ...
    public void init() {
        ...
        Object value = this.library.getObject(this.entries, STATE_KEY);
        if (value != null && (value instanceof StringObject)) {
            StringObject text = (StringObject) value;
            this.state = text.getDecryptedLiteralString(this.securityManager);
        } else if (value instanceof String) {
            this.state = (String) value;
        }
        Object value2 = this.library.getName(this.entries, NAME_KEY);
        if (value2 != null) {
            this.iconName = (Name) value2;
        }
        Object value3 = this.library.getObject(this.entries, STATE_MODEL_KEY);
        if (value3 != null && (value3 instanceof StringObject)) {
            StringObject text2 = (StringObject) value3;
            this.stateModel = text2.getDecryptedLiteralString(this.securityManager);
        } else if (value3 instanceof String) {
            this.stateModel = (String) value3;
        }
    }

    public static TextAnnotation getInstance(Library library, Rectangle rect) {
        ...
        if (rect != null) {
            ...
            entries.put(Annotation.RECTANGLE_KEY, new Rectangle2D.Float(10.0f, 10.0f, 50.0f, 100.0f));
        }
        ...
        textAnnotation.setFlag(64, false);
        textAnnotation.setFlag(16, true);
        textAnnotation.setFlag(8, true);
        textAnnotation.setFlag(4, true);
    ...
    public void resetAppearanceStream(double dx, double dy, AffineTransform pageTransform) {
        ...
        if (this.iconName.equals(COMMENT_ICON)) {
            iconContentString = COMMENT_CONTENT_STREAM;
        } else if (this.iconName.equals(CHECK_ICON)) {
            iconContentString = CHECK_CONTENT_STREAM;
        } else if (this.iconName.equals(CHECK_MARK_ICON)) {
            iconContentString = CHECK_MARK_CONTENT_STREAM;
        } else if (this.iconName.equals(CIRCLE_ICON)) {
            iconContentString = CIRCLE_CONTENT_STREAM;
        } else if (this.iconName.equals(CROSS_ICON)) {
            iconContentString = CROSS_CONTENT_STREAM;
        } else if (this.iconName.equals(CROSS_HAIRS_ICON)) {
            iconContentString = CROSS_HAIRS_CONTENT_STREAM;
        } else if (this.iconName.equals(HELP_ICON)) {
            iconContentString = HELP_CONTENT_STREAM;
        } else if (this.iconName.equals(INSERT_ICON)) {
            iconContentString = INSERT_CONTENT_STREAM;
        } else if (this.iconName.equals(KEY_ICON)) {
            iconContentString = KEY_CONTENT_STREAM;
        } else if (this.iconName.equals(NEW_PARAGRAPH_ICON)) {
            iconContentString = NEW_PARAGRAPH_CONTENT_STREAM;
        } else if (this.iconName.equals(PARAGRAPH_ICON)) {
            iconContentString = PARAGRAPH_CONTENT_STREAM;
        } else if (this.iconName.equals(RIGHT_ARROW_ICON)) {
            iconContentString = RIGHT_ARROW_CONTENT_STREAM;
        } else if (this.iconName.equals(RIGHT_POINTER_ICON)) {
            iconContentString = RIGHT_POINTER_CONTENT_STREAM;
        } else if (this.iconName.equals(STAR_ICON)) {
            iconContentString = STAR_CONTENT_STREAM;
        } else if (this.iconName.equals(UP_ARROW_ICON)) {
            iconContentString = UP_ARROW_CONTENT_STREAM;
        } else if (this.iconName.equals(UP_LEFT_ARROW_ICON)) {
        ...
        StringBuilder colorString = new StringBuilder().append(compArray[0]).append(" ").append(compArray[1]).append(" ").append(compArray[2]);
        Object[] colorArgument = {colorString};
        MessageFormat formatter = new MessageFormat(iconContentString);
        String iconContentString2 = formatter.format(colorArgument);
        try {
            this.shapes = ContentParserFactory.getInstance().getContentParser(this.library, null).parse(new byte[]{iconContentString2.getBytes()}, null).getShapes();
        } catch (Exception e2) {
            this.shapes = new Shapes();
            logger.log(Level.FINE, "Error building named icon.", (Throwable) e2);
        }
        ...
        if (form != null) {
            ...
            form.setRawBytes(iconContentString2.getBytes());
    ...
    public void setIconName(Name iconName) {
        ...
        this.entries.put(NAME_KEY, iconName);
    }

    public void setState(String state) {
        this.state = state;
        this.entries.put(STATE_KEY, new LiteralStringObject(state));
    }

    public void setStateModel(String stateModel) {
        this.stateModel = stateModel;
        this.entries.put(STATE_MODEL_KEY, new LiteralStringObject(stateModel));
    }
}

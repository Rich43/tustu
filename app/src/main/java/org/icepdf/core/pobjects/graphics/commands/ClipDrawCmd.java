...
/* loaded from: icepdf-core.jar:org/icepdf/core/pobjects/graphics/commands/ClipDrawCmd.class */
public class ClipDrawCmd extends AbstractDrawCmd {
    ...
    public Shape paintOperand(Graphics2D g2, Page parentPage, Shape currentShape, Shape clip, AffineTransform base, OptionalContentState optionalContentState, boolean paintAlpha, PaintTimer paintTimer) {
        AffineTransform af2 = new AffineTransform(g2.getTransform());
        g2.setTransform(base);
        g2.setClip(clip);
        g2.setTransform(af2);
        if (currentShape != null && !disableClipping) {
            g2.clip(currentShape);
        }
        return currentShape;
    }
}

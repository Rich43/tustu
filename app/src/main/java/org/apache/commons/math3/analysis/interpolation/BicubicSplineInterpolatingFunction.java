...
@Deprecated
/* loaded from: commons-math3-3.6.1.jar:org/apache/commons/math3/analysis/interpolation/BicubicSplineInterpolatingFunction.class */
public class BicubicSplineInterpolatingFunction implements BivariateFunction {
    ...
    public BicubicSplineInterpolatingFunction(double[] x2, double[] y2, double[][] f2, double[][] dFdX, double[][] dFdY, double[][] d2FdXdY) throws NoDataException, DimensionMismatchException, NonMonotonicSequenceException {
        this(x2, y2, f2, dFdX, dFdY, d2FdXdY, false);
    }

    public BicubicSplineInterpolatingFunction(double[] x2, double[] y2, double[][] f2, double[][] dFdX, double[][] dFdY, double[][] d2FdXdY, boolean initializeDerivatives) throws NoDataException, NonMonotonicSequenceException, DimensionMismatchException {
        int xLen = x2.length;
        int yLen = y2.length;
        ...
        for (int i2 = 0; i2 < lastI; i2++) {
            ...
            for (int j2 = 0; j2 < lastJ; j2++) {
                int jp1 = j2 + 1;
                double[] beta = {f2[i2][j2], f2[ip1][j2], f2[i2][jp1], f2[ip1][jp1], dFdX[i2][j2], dFdX[ip1][j2], dFdX[i2][jp1], dFdX[ip1][jp1], dFdY[i2][j2], dFdY[ip1][j2], dFdY[i2][jp1], dFdY[ip1][jp1], d2FdXdY[i2][j2], d2FdXdY[ip1][j2], d2FdXdY[i2][jp1], d2FdXdY[ip1][jp1]};
                this.splines[i2][j2] = new BicubicSplineFunction(computeSplineCoefficients(beta), initializeDerivatives);
            }
        }
        if (initializeDerivatives) {
            ...
            for (int i3 = 0; i3 < lastI; i3++) {
                for (int j3 = 0; j3 < lastJ; j3++) {
                    BicubicSplineFunction bcs = this.splines[i3][j3];
                    this.partialDerivatives[0][i3][j3] = bcs.partialDerivativeX();
                    this.partialDerivatives[1][i3][j3] = bcs.partialDerivativeY();
                    this.partialDerivatives[2][i3][j3] = bcs.partialDerivativeXX();
                    this.partialDerivatives[3][i3][j3] = bcs.partialDerivativeYY();
                    this.partialDerivatives[4][i3][j3] = bcs.partialDerivativeXY();
                }
            }
            return;
        }
    ...
    public double partialDerivativeX(double x2, double y2) throws OutOfRangeException {
        return partialDerivative(0, x2, y2);
    }

    public double partialDerivativeY(double x2, double y2) throws OutOfRangeException {
        return partialDerivative(1, x2, y2);
    }

    public double partialDerivativeXX(double x2, double y2) throws OutOfRangeException {
        return partialDerivative(2, x2, y2);
    }

    public double partialDerivativeYY(double x2, double y2) throws OutOfRangeException {
        return partialDerivative(3, x2, y2);
    }

    public double partialDerivativeXY(double x2, double y2) throws OutOfRangeException {
        return partialDerivative(4, x2, y2);
    }

    private double partialDerivative(int which, double x2, double y2) throws OutOfRangeException {
        int i2 = searchIndex(x2, this.xval);
        int j2 = searchIndex(y2, this.yval);
        double xN = (x2 - this.xval[i2]) / (this.xval[i2 + 1] - this.xval[i2]);
        double yN = (y2 - this.yval[j2]) / (this.yval[j2 + 1] - this.yval[j2]);

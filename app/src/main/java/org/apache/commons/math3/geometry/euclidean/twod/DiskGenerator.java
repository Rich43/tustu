...
/* loaded from: commons-math3-3.6.1.jar:org/apache/commons/math3/geometry/euclidean/twod/DiskGenerator.class */
public class DiskGenerator implements SupportBallGenerator<Euclidean2D, Vector2D> {
    ...
    public EnclosingBall<Euclidean2D, Vector2D> ballOnSupport(List<Vector2D> support) {
        ...
        if (support.size() < 3) {
            return new EnclosingBall<>(new Vector2D(0.5d, vA, 0.5d, vB), 0.5d * vA.distance((Vector<Euclidean2D>) vB), vA, vB);
        }

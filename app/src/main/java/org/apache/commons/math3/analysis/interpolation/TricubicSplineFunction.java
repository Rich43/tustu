...
/* loaded from: commons-math3-3.6.1.jar:org/apache/commons/math3/analysis/interpolation/TricubicSplineFunction.class */
class TricubicSplineFunction implements TrivariateFunction {

    /* renamed from: N, reason: collision with root package name */
    private static final short f12972N = 4;

    /* renamed from: a, reason: collision with root package name */
    private final double[][][] f12973a = new double[4][4][4];

    TricubicSplineFunction(double[] aV2) {
        for (int i2 = 0; i2 < 4; i2++) {
            for (int j2 = 0; j2 < 4; j2++) {
                for (int k2 = 0; k2 < 4; k2++) {
                    this.f12973a[i2][j2][k2] = aV2[i2 + (4 * (j2 + (4 * k2)))];
                }
            }
        }
    }
    ...
    public double value(double x2, double y2, double z2) throws OutOfRangeException {
        ...
        for (int i2 = 0; i2 < 4; i2++) {
            for (int j2 = 0; j2 < 4; j2++) {
                for (int k2 = 0; k2 < 4; k2++) {
                    result += this.f12973a[i2][j2][k2] * pX[i2] * pY[j2] * pZ[k2];
                }
            }
        }
        return result;

...
/* loaded from: commons-math3-3.6.1.jar:org/apache/commons/math3/geometry/euclidean/threed/SphereGenerator.class */
public class SphereGenerator implements SupportBallGenerator<Euclidean3D, Vector3D> {
    ...
    public EnclosingBall<Euclidean3D, Vector3D> ballOnSupport(List<Vector3D> support) {
        ...
        if (support.size() < 4) {
            Plane p2 = new Plane(vA, vB, vC, 1.0E-10d * (vA.getNorm1() + vB.getNorm1() + vC.getNorm1()));
            EnclosingBall<Euclidean2D, Vector2D> disk = new DiskGenerator().ballOnSupport(Arrays.asList(p2.toSubSpace((Vector<Euclidean3D>) vA), p2.toSubSpace((Vector<Euclidean3D>) vB), p2.toSubSpace((Vector<Euclidean3D>) vC)));
            return new EnclosingBall<>(p2.toSpace((Vector<Euclidean2D>) disk.getCenter()), disk.getRadius(), vA, vB, vC);
        }

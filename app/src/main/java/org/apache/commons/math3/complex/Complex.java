...
/* loaded from: commons-math3-3.6.1.jar:org/apache/commons/math3/complex/Complex.class */
public class Complex implements FieldElement<Complex>, Serializable {

    /* renamed from: I, reason: collision with root package name */
    public static final Complex f12981I = new Complex(0.0d, 1.0d);
    ...
    public double abs() {
        ...
        if (FastMath.abs(this.real) < FastMath.abs(this.imaginary)) {
            ...
            double q2 = this.real / this.imaginary;
            return FastMath.abs(this.imaginary) * FastMath.sqrt(1.0d + (q2 * q2));
        }
        if (this.real == 0.0d) {
            return FastMath.abs(this.imaginary);
        }
        double q3 = this.imaginary / this.real;
        return FastMath.abs(this.real) * FastMath.sqrt(1.0d + (q3 * q3));
    }
    ...
    public Complex divide(Complex divisor) throws NullArgumentException {
        ...
        if (FastMath.abs(c2) < FastMath.abs(d2)) {
            double q2 = c2 / d2;
            double denominator = (c2 * q2) + d2;
            return createComplex(((this.real * q2) + this.imaginary) / denominator, ((this.imaginary * q2) - this.real) / denominator);
        }
        double q3 = d2 / c2;
        double denominator2 = (d2 * q3) + c2;
        return createComplex(((this.imaginary * q3) + this.real) / denominator2, (this.imaginary - (this.real * q3)) / denominator2);
    }
    ...
    public Complex reciprocal() {
        ...
        if (FastMath.abs(this.real) < FastMath.abs(this.imaginary)) {
            double q2 = this.real / this.imaginary;
            double scale = 1.0d / ((this.real * q2) + this.imaginary);
            return createComplex(scale * q2, -scale);
        }
        double q3 = this.imaginary / this.real;
        double scale2 = 1.0d / ((this.imaginary * q3) + this.real);
        return createComplex(scale2, (-scale2) * q3);
    }
    ...
    public static boolean equals(Complex x2, Complex y2, int maxUlps) {
        return Precision.equals(x2.real, y2.real, maxUlps) && Precision.equals(x2.imaginary, y2.imaginary, maxUlps);
    }

    public static boolean equals(Complex x2, Complex y2) {
        return equals(x2, y2, 1);
    }

    public static boolean equals(Complex x2, Complex y2, double eps) {
        return Precision.equals(x2.real, y2.real, eps) && Precision.equals(x2.imaginary, y2.imaginary, eps);
    }

    public static boolean equalsWithRelativeTolerance(Complex x2, Complex y2, double eps) {
        return Precision.equalsWithRelativeTolerance(x2.real, y2.real, eps) && Precision.equalsWithRelativeTolerance(x2.imaginary, y2.imaginary, eps);
    }
    ...
    public Complex acos() {
        ...
        return add(sqrt1z().multiply(f12981I)).log().multiply(f12981I.negate());
    }
    ...
    public Complex asin() {
        ...
        return sqrt1z().add(multiply(f12981I)).log().multiply(f12981I.negate());
    }
    ...
    public Complex atan() {
        ...
        return add(f12981I).divide(f12981I.subtract(this)).log().multiply(f12981I.divide(createComplex(2.0d, 0.0d)));
    }
    ...
    public Complex pow(Complex x2) throws NullArgumentException {
        MathUtils.checkNotNull(x2);
        return log().multiply(x2).exp();
    }

    public Complex pow(double x2) {
        return log().multiply(x2).exp();
    }
    ...
    public Complex sqrt() {
        ...
        double t2 = FastMath.sqrt((FastMath.abs(this.real) + abs()) / 2.0d);
        if (this.real >= 0.0d) {
            return createComplex(t2, this.imaginary / (2.0d * t2));
        }
        return createComplex(FastMath.abs(this.imaginary) / (2.0d * t2), FastMath.copySign(1.0d, this.imaginary) * t2);
    }
    ...
    public Complex tan() {
        ...
        double d2 = FastMath.cos(real2) + FastMath.cosh(imaginary2);
        return createComplex(FastMath.sin(real2) / d2, FastMath.sinh(imaginary2) / d2);
    }
    ...
    public Complex tanh() {
        ...
        double real2 = 2.0d * this.real;
        double imaginary2 = 2.0d * this.imaginary;
        double d2 = FastMath.cosh(real2) + FastMath.cos(imaginary2);
        return createComplex(FastMath.sinh(real2) / d2, FastMath.sin(imaginary2) / d2);
    }
    ...
    public List<Complex> nthRoot(int n2) throws NotPositiveException {
        if (n2 <= 0) {
            throw new NotPositiveException(LocalizedFormats.CANNOT_COMPUTE_NTH_ROOT_FOR_NEGATIVE_N, Integer.valueOf(n2));
        }
        ...
        double nthRootOfAbs = FastMath.pow(abs(), 1.0d / n2);
        double nthPhi = getArgument() / n2;
        double slice = 6.283185307179586d / n2;

...
/* loaded from: commons-math3-3.6.1.jar:org/apache/commons/math3/analysis/interpolation/TricubicInterpolatingFunction.class */
public class TricubicInterpolatingFunction implements TrivariateFunction {
    ...
    public TricubicInterpolatingFunction(double[] x2, double[] y2, double[] z2, double[][][] f2, double[][][] dFdX, double[][][] dFdY, double[][][] dFdZ, double[][][] d2FdXdY, double[][][] d2FdXdZ, double[][][] d2FdYdZ, double[][][] d3FdXdYdZ) throws NoDataException, NonMonotonicSequenceException, DimensionMismatchException {
        int xLen = x2.length;
        int yLen = y2.length;
        int zLen = z2.length;
        ...
        for (int i2 = 0; i2 < lastI; i2++) {
            ...
            for (int j2 = 0; j2 < lastJ; j2++) {
                ...
                for (int k2 = 0; k2 < lastK; k2++) {
                    int kp1 = k2 + 1;
                    double zR = this.zval[kp1] - this.zval[k2];
                    double xRzR = xR * zR;
                    double yRzR = yR * zR;
                    double xRyRzR = xR * yRzR;
                    double[] beta = {f2[i2][j2][k2], f2[ip1][j2][k2], f2[i2][jp1][k2], f2[ip1][jp1][k2], f2[i2][j2][kp1], f2[ip1][j2][kp1], f2[i2][jp1][kp1], f2[ip1][jp1][kp1], dFdX[i2][j2][k2] * xR, dFdX[ip1][j2][k2] * xR, dFdX[i2][jp1][k2] * xR, dFdX[ip1][jp1][k2] * xR, dFdX[i2][j2][kp1] * xR, dFdX[ip1][j2][kp1] * xR, dFdX[i2][jp1][kp1] * xR, dFdX[ip1][jp1][kp1] * xR, dFdY[i2][j2][k2] * yR, dFdY[ip1][j2][k2] * yR, dFdY[i2][jp1][k2] * yR, dFdY[ip1][jp1][k2] * yR, dFdY[i2][j2][kp1] * yR, dFdY[ip1][j2][kp1] * yR, dFdY[i2][jp1][kp1] * yR, dFdY[ip1][jp1][kp1] * yR, dFdZ[i2][j2][k2] * zR, dFdZ[ip1][j2][k2] * zR, dFdZ[i2][jp1][k2] * zR, dFdZ[ip1][jp1][k2] * zR, dFdZ[i2][j2][kp1] * zR, dFdZ[ip1][j2][kp1] * zR, dFdZ[i2][jp1][kp1] * zR, dFdZ[ip1][jp1][kp1] * zR, d2FdXdY[i2][j2][k2] * xRyR, d2FdXdY[ip1][j2][k2] * xRyR, d2FdXdY[i2][jp1][k2] * xRyR, d2FdXdY[ip1][jp1][k2] * xRyR, d2FdXdY[i2][j2][kp1] * xRyR, d2FdXdY[ip1][j2][kp1] * xRyR, d2FdXdY[i2][jp1][kp1] * xRyR, d2FdXdY[ip1][jp1][kp1] * xRyR, d2FdXdZ[i2][j2][k2] * xRzR, d2FdXdZ[ip1][j2][k2] * xRzR, d2FdXdZ[i2][jp1][k2] * xRzR, d2FdXdZ[ip1][jp1][k2] * xRzR, d2FdXdZ[i2][j2][kp1] * xRzR, d2FdXdZ[ip1][j2][kp1] * xRzR, d2FdXdZ[i2][jp1][kp1] * xRzR, d2FdXdZ[ip1][jp1][kp1] * xRzR, d2FdYdZ[i2][j2][k2] * yRzR, d2FdYdZ[ip1][j2][k2] * yRzR, d2FdYdZ[i2][jp1][k2] * yRzR, d2FdYdZ[ip1][jp1][k2] * yRzR, d2FdYdZ[i2][j2][kp1] * yRzR, d2FdYdZ[ip1][j2][kp1] * yRzR, d2FdYdZ[i2][jp1][kp1] * yRzR, d2FdYdZ[ip1][jp1][kp1] * yRzR, d3FdXdYdZ[i2][j2][k2] * xRyRzR, d3FdXdYdZ[ip1][j2][k2] * xRyRzR, d3FdXdYdZ[i2][jp1][k2] * xRyRzR, d3FdXdYdZ[ip1][jp1][k2] * xRyRzR, d3FdXdYdZ[i2][j2][kp1] * xRyRzR, d3FdXdYdZ[ip1][j2][kp1] * xRyRzR, d3FdXdYdZ[i2][jp1][kp1] * xRyRzR, d3FdXdYdZ[ip1][jp1][kp1] * xRyRzR};
                    this.splines[i2][j2][k2] = new TricubicFunction(computeCoefficients(beta));
                }
            }
        }
    }
    ...
    public boolean isValidPoint(double x2, double y2, double z2) {
        if (x2 < this.xval[0] || x2 > this.xval[this.xval.length - 1] || y2 < this.yval[0] || y2 > this.yval[this.yval.length - 1] || z2 < this.zval[0] || z2 > this.zval[this.zval.length - 1]) {
            return false;
        }
        return true;
    }

...
/* loaded from: commons-math3-3.6.1.jar:org/apache/commons/math3/distribution/MixtureMultivariateRealDistribution.class */
public class MixtureMultivariateRealDistribution<T extends MultivariateRealDistribution> extends AbstractMultivariateRealDistribution {
    ...
    public MixtureMultivariateRealDistribution(RandomGenerator rng, List<Pair<Double, T>> components) {
        ...
        for (int i3 = 0; i3 < numComp; i3++) {
            Pair<Double, T> comp2 = components.get(i3);
            this.weight[i3] = comp2.getFirst().doubleValue() / weightSum;
    ...
    public double density(double[] values) {
        ...
        for (int i2 = 0; i2 < this.weight.length; i2++) {
            p2 += this.weight[i2] * this.distribution.get(i2).density(values);
        }
        return p2;
    }
    ...
    public double[] sample() {
        ...
        while (true) {
            ...
            if (randomValue > sum) {
                ...
                vals = this.distribution.get(i2).sample();
                break;
            }
        }
        if (vals == null) {
    ...
    public void reseedRandomGenerator(long seed) {
        ...
        for (int i2 = 0; i2 < this.distribution.size(); i2++) {
            this.distribution.get(i2).reseedRandomGenerator(i2 + 1 + seed);
        }
    }
    ...
    public List<Pair<Double, T>> getComponents() {
        ...
        for (int i2 = 0; i2 < this.weight.length; i2++) {
            list.add(new Pair<>(Double.valueOf(this.weight[i2]), this.distribution.get(i2)));
        }
        return list;
    }
}

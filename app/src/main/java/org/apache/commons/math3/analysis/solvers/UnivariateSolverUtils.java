...
/* loaded from: commons-math3-3.6.1.jar:org/apache/commons/math3/analysis/solvers/UnivariateSolverUtils.class */
public class UnivariateSolverUtils {
    ...
    public static double forceSide(int maxEval, UnivariateFunction f2, BracketedUnivariateSolver<UnivariateFunction> bracketing, double baseRoot, double min, double max, AllowedSolution allowedSolution) throws NoBracketingException {
    ...
    public static double[] bracket(UnivariateFunction function, double initial, double lowerBound, double upperBound) throws NotStrictlyPositiveException, NullArgumentException, NoBracketingException {
        return bracket(function, initial, lowerBound, upperBound, 1.0d, 1.0d, Integer.MAX_VALUE);
    }

    public static double[] bracket(UnivariateFunction function, double initial, double lowerBound, double upperBound, int maximumIterations) throws NotStrictlyPositiveException, NullArgumentException, NoBracketingException {
        return bracket(function, initial, lowerBound, upperBound, 1.0d, 1.0d, maximumIterations);
    }

    public static double[] bracket(UnivariateFunction function, double initial, double lowerBound, double upperBound, double q2, double r2, int maximumIterations) throws NumberIsTooLargeException, NoBracketingException {
        if (function == null) {
            throw new NullArgumentException(LocalizedFormats.FUNCTION, new Object[0]);
        }
        if (q2 <= 0.0d) {
            throw new NotStrictlyPositiveException(Double.valueOf(q2));
        }
        ...
        for (int numIterations = 0; numIterations < maximumIterations && (a2 > lowerBound || b2 < upperBound); numIterations++) {
            ...
            if (numIterations == 0) {
                ...
                if (fa * previousFa <= 0.0d) {
                    return new double[]{a2, previousA};
                }
                if (fb * previousFb <= 0.0d) {

...
/* loaded from: commons-math3-3.6.1.jar:org/apache/commons/math3/distribution/EnumeratedIntegerDistribution.class */
public class EnumeratedIntegerDistribution extends AbstractIntegerDistribution {
    ...
    public EnumeratedIntegerDistribution(int[] singletons, double[] probabilities) throws NotPositiveException, NotANumberException, NotFiniteNumberException, DimensionMismatchException, MathArithmeticException {
        this(new Well19937c(), singletons, probabilities);
    }

    public EnumeratedIntegerDistribution(RandomGenerator rng, int[] singletons, double[] probabilities) throws NotPositiveException, NotANumberException, NotFiniteNumberException, DimensionMismatchException, MathArithmeticException {
        super(rng);
    ...
    private static List<Pair<Integer, Double>> createDistribution(int[] singletons, double[] probabilities) {
        ...
        for (int i2 = 0; i2 < singletons.length; i2++) {
            samples.add(new Pair<>(Integer.valueOf(singletons[i2]), Double.valueOf(probabilities[i2])));
        }
        return samples;
    }
    ...
    public double probability(int x2) {
        return this.innerDistribution.probability(Integer.valueOf(x2));
    }
    ...
    public double getNumericalMean() {
        double mean = 0.0d;
        for (Pair<Integer, Double> sample : this.innerDistribution.getPmf()) {
            mean += sample.getValue().doubleValue() * sample.getKey().intValue();
        }
        return mean;
    }
    ...
    public double getNumericalVariance() {
        ...
        for (Pair<Integer, Double> sample : this.innerDistribution.getPmf()) {
            mean += sample.getValue().doubleValue() * sample.getKey().intValue();
            meanOfSquares += sample.getValue().doubleValue() * sample.getKey().intValue() * sample.getKey().intValue();
        }

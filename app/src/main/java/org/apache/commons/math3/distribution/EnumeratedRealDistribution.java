...
/* loaded from: commons-math3-3.6.1.jar:org/apache/commons/math3/distribution/EnumeratedRealDistribution.class */
public class EnumeratedRealDistribution extends AbstractRealDistribution {
    private static final long serialVersionUID = 20130308;
    protected final EnumeratedDistribution<Double> innerDistribution;

    public EnumeratedRealDistribution(double[] singletons, double[] probabilities) throws NotPositiveException, NotANumberException, NotFiniteNumberException, DimensionMismatchException, MathArithmeticException {
        this(new Well19937c(), singletons, probabilities);
    }

    public EnumeratedRealDistribution(RandomGenerator rng, double[] singletons, double[] probabilities) throws NotPositiveException, NotANumberException, NotFiniteNumberException, DimensionMismatchException, MathArithmeticException {
        super(rng);
    ...
    private static List<Pair<Double, Double>> createDistribution(double[] singletons, double[] probabilities) {
        ...
        for (int i2 = 0; i2 < singletons.length; i2++) {
            samples.add(new Pair<>(Double.valueOf(singletons[i2]), Double.valueOf(probabilities[i2])));
        }
        return samples;
    }
    ...
    public double probability(double x2) {
        return this.innerDistribution.probability(Double.valueOf(x2));
    }

    @Override // org.apache.commons.math3.distribution.RealDistribution
    public double density(double x2) {
        return probability(x2);
    }
    ...
    public double cumulativeProbability(double x2) {
        ...
        for (Pair<Double, Double> sample : this.innerDistribution.getPmf()) {
            if (sample.getKey().doubleValue() <= x2) {
    ...
    public double inverseCumulativeProbability(double p2) throws OutOfRangeException {
        ...
        double probability = 0.0d;
        double x2 = getSupportLowerBound();
        for (Pair<Double, Double> sample : this.innerDistribution.getPmf()) {
            if (sample.getValue().doubleValue() != 0.0d) {
                probability += sample.getValue().doubleValue();
                x2 = sample.getKey().doubleValue();
                if (probability >= p2) {
                    break;
                }
            }
    ...
    public double getNumericalMean() {
        double mean = 0.0d;
        for (Pair<Double, Double> sample : this.innerDistribution.getPmf()) {
            mean += sample.getValue().doubleValue() * sample.getKey().doubleValue();
        }
        return mean;
    }
    ...
    public double getNumericalVariance() {
        ...
        for (Pair<Double, Double> sample : this.innerDistribution.getPmf()) {
            mean += sample.getValue().doubleValue() * sample.getKey().doubleValue();
            meanOfSquares += sample.getValue().doubleValue() * sample.getKey().doubleValue() * sample.getKey().doubleValue();
        }

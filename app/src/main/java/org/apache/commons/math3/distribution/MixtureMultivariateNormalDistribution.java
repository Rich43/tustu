...
/* loaded from: commons-math3-3.6.1.jar:org/apache/commons/math3/distribution/MixtureMultivariateNormalDistribution.class */
public class MixtureMultivariateNormalDistribution extends MixtureMultivariateRealDistribution<MultivariateNormalDistribution> {
    ...
    private static List<Pair<Double, MultivariateNormalDistribution>> createComponents(double[] weights, double[][] means, double[][][] covariances) {
        ...
        for (int i2 = 0; i2 < weights.length; i2++) {
            MultivariateNormalDistribution dist = new MultivariateNormalDistribution(means[i2], covariances[i2]);
            mvns.add(new Pair<>(Double.valueOf(weights[i2]), dist));
        }
        return mvns;
    }
}

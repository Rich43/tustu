...
/* loaded from: commons-net-3.6.jar:org/apache/commons/net/ftp/FTPListParseEngine.class */
public class FTPListParseEngine {
    ...
    private void readStream(InputStream stream, String encoding) throws IOException {
        ...
        String nextEntry = this.parser.readNextEntry(reader);
        while (true) {
            String line = nextEntry;
            if (line != null) {
                this.entries.add(line);
                nextEntry = this.parser.readNextEntry(reader);
            } else {
                reader.close();
                return;
            }

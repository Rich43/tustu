...
/* loaded from: commons-net-3.6.jar:org/apache/commons/net/ftp/FTP.class */
public class FTP extends SocketClient {
        ...
        try {
            ...
            if (length > 3) {
                ...
                if (sep == '-') {
                    ...
                    if (length == 4) {
                        throw new MalformedServerReplyException("Truncated server reply: '" + line + PdfOps.SINGLE_QUOTE_TOKEN);
                    }
                    if (sep != ' ') {
                        throw new MalformedServerReplyException("Invalid server reply: '" + line + PdfOps.SINGLE_QUOTE_TOKEN);
                    }
                }
            } else if (isStrictReplyParsing()) {
                throw new MalformedServerReplyException("Truncated server reply: '" + line + PdfOps.SINGLE_QUOTE_TOKEN);
    ...
    public int eprt(InetAddress host, int port) throws IOException {
        ...
        String h2 = host.getHostAddress();
        int num = h2.indexOf(FXMLLoader.RESOURCE_KEY_PREFIX);
        if (num > 0) {
            h2 = h2.substring(0, num);
        }
        info.append(CallSiteDescriptor.OPERATOR_DELIMITER);

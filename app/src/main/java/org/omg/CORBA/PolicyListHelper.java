package org.omg.CORBA;

import org.omg.CORBA.portable.InputStream;
import org.omg.CORBA.portable.OutputStream;

/* loaded from: rt.jar:org/omg/CORBA/PolicyListHelper.class */
public abstract class PolicyListHelper {
    private static String _id = "IDL:omg.org/CORBA/PolicyList:1.0";
    private static TypeCode __typeCode = null;

    public static void insert(Any any, Policy[] policyArr) throws MARSHAL {
        OutputStream outputStreamCreate_output_stream = any.create_output_stream();
        any.type(type());
        write(outputStreamCreate_output_stream, policyArr);
        any.read_value(outputStreamCreate_output_stream.create_input_stream(), type());
    }

    public static Policy[] extract(Any any) {
        return read(any.create_input_stream());
    }

    public static synchronized TypeCode type() {
        if (__typeCode == null) {
            __typeCode = PolicyHelper.type();
            __typeCode = ORB.init().create_sequence_tc(0, __typeCode);
            __typeCode = ORB.init().create_alias_tc(id(), "PolicyList", __typeCode);
        }
        return __typeCode;
    }

    public static String id() {
        return _id;
    }

    public static Policy[] read(InputStream inputStream) {
        Policy[] policyArr = new Policy[inputStream.read_long()];
        for (int i2 = 0; i2 < policyArr.length; i2++) {
            policyArr[i2] = PolicyHelper.read(inputStream);
        }
        return policyArr;
    }

    public static void write(OutputStream outputStream, Policy[] policyArr) {
        outputStream.write_long(policyArr.length);
        for (Policy policy : policyArr) {
            PolicyHelper.write(outputStream, policy);
        }
    }
}

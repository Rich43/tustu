...
/* loaded from: rt.jar:org/omg/PortableServer/ForwardRequestHelper.class */
public abstract class ForwardRequestHelper {
        ...
        if (__typeCode == null) {
            synchronized (TypeCode.class) {
                if (__typeCode == null) {
                    ...
                    __active = true;
                    __typeCode = ORB.init().create_exception_tc(id(), "ForwardRequest", new StructMember[]{new StructMember("forward_reference", ObjectHelper.type(), null)});
                    __active = false;
                }
            }
        }
    ...
    public static ForwardRequest read(InputStream inputStream) {
        ...
        forwardRequest.forward_reference = ObjectHelper.read(inputStream);
        return forwardRequest;
    }

    public static void write(OutputStream outputStream, ForwardRequest forwardRequest) {
        outputStream.write_string(id());
        ObjectHelper.write(outputStream, forwardRequest.forward_reference);
    }
}

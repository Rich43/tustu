...
/* loaded from: rt.jar:javax/swing/plaf/multi/MultiListUI.class */
public class MultiListUI extends ListUI {
    ...
    public int locationToIndex(JList jList, Point point) {
        int iLocationToIndex = ((ListUI) this.uis.elementAt(0)).locationToIndex(jList, point);
        for (int i2 = 1; i2 < this.uis.size(); i2++) {
            ((ListUI) this.uis.elementAt(i2)).locationToIndex(jList, point);
        }
    ...
    public Point indexToLocation(JList jList, int i2) {
        Point pointIndexToLocation = ((ListUI) this.uis.elementAt(0)).indexToLocation(jList, i2);
        for (int i3 = 1; i3 < this.uis.size(); i3++) {
            ((ListUI) this.uis.elementAt(i3)).indexToLocation(jList, i2);
        }
    ...
    public Rectangle getCellBounds(JList jList, int i2, int i3) {
        Rectangle cellBounds = ((ListUI) this.uis.elementAt(0)).getCellBounds(jList, i2, i3);
        for (int i4 = 1; i4 < this.uis.size(); i4++) {
            ((ListUI) this.uis.elementAt(i4)).getCellBounds(jList, i2, i3);
        }
        return cellBounds;
    }

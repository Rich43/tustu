...
/* loaded from: rt.jar:javax/swing/plaf/nimbus/MenuPainter.class */
final class MenuPainter extends AbstractRegionPainter {
    ...
    public MenuPainter(AbstractRegionPainter.PaintContext paintContext, int i2) {
        this.state = i2;
    ...
    protected void doPaint(Graphics2D graphics2D, JComponent jComponent, int i2, int i3, Object[] objArr) {
        ...
        switch (this.state) {
            case 3:
                ...
                break;
            case 4:
                paintarrowIconDisabled(graphics2D);
                break;
            case 5:
                paintarrowIconEnabled(graphics2D);
                break;
            case 6:
                paintarrowIconEnabledAndSelected(graphics2D);
                break;
        }
    }
    ...
    private void paintarrowIconDisabled(Graphics2D graphics2D) {
        this.path = decodePath1();
        graphics2D.setPaint(this.color2);
        graphics2D.fill(this.path);
    }

    private void paintarrowIconEnabled(Graphics2D graphics2D) {
        this.path = decodePath1();
        graphics2D.setPaint(this.color3);
        graphics2D.fill(this.path);
    }

    private void paintarrowIconEnabledAndSelected(Graphics2D graphics2D) {

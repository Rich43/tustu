...
/* loaded from: rt.jar:javax/swing/plaf/synth/Region.class */
public class Region {
    ...
    private static Map<String, Region> getUItoRegionMap() {
        ...
        Map<String, Region> map = (Map) appContext.get(UI_TO_REGION_MAP_KEY);
        if (map == null) {
            map = new HashMap();
    ...
    private static Map<Region, String> getLowerCaseNameMap() {
        AppContext appContext = AppContext.getAppContext();
        Map<Region, String> map = (Map) appContext.get(LOWER_CASE_NAME_MAP_KEY);
        if (map == null) {
            map = new HashMap();
    ...
    static Region getRegion(JComponent jComponent) {
        return getUItoRegionMap().get(jComponent.getUIClassID());
    }

    static void registerUIs(UIDefaults uIDefaults) {
        Iterator<String> it = getUItoRegionMap().keySet().iterator();
        while (it.hasNext()) {
            uIDefaults.put(it.next(), "javax.swing.plaf.synth.SynthLookAndFeel");
        }
    }

    private Region(String str, boolean z2) {
        if (str == null) {
    ...
    protected Region(String str, String str2, boolean z2) {
        this(str, z2);
        if (str2 != null) {
            getUItoRegionMap().put(str2, this);
        }
    }

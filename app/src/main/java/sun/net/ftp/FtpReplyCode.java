...
/* loaded from: rt.jar:sun/net/ftp/FtpReplyCode.class */
public enum FtpReplyCode {
    ...
    NEED_ADAT(FTPReply.SECURITY_MECHANISM_IS_OK),
    NEED_MORE_ADAT(335),
    FILE_ACTION_PENDING(FTPReply.FILE_ACTION_PENDING),
    SERVICE_NOT_AVAILABLE(421),
    CANT_OPEN_DATA_CONNECTION(FTPReply.CANNOT_OPEN_DATA_CONNECTION),
    CONNECTION_CLOSED(FTPReply.TRANSFER_ABORTED),
    ...
    UNSUPPORTED_PROT_LEVEL(FTPReply.REQUESTED_PROT_LEVEL_NOT_SUPPORTED),
    ...
    public static FtpReplyCode find(int i2) {
        for (FtpReplyCode ftpReplyCode : values()) {
            if (ftpReplyCode.getValue() == i2) {
                return ftpReplyCode;
            }
        }

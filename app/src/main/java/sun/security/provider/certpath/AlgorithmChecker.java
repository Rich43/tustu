...
/* loaded from: rt.jar:sun/security/provider/certpath/AlgorithmChecker.class */
public final class AlgorithmChecker extends PKIXCertPathChecker {
    ...
    public AlgorithmChecker(TrustAnchor trustAnchor, String str) {
        this(trustAnchor, certPathDefaultConstraints, null, str);
    }

    public AlgorithmChecker(AlgorithmConstraints algorithmConstraints, String str) {
        this(null, algorithmConstraints, null, str);
    }

    public AlgorithmChecker(TrustAnchor trustAnchor, AlgorithmConstraints algorithmConstraints, Date date, String str) {
        if (trustAnchor != null) {
    ...
    public AlgorithmChecker(TrustAnchor trustAnchor, Date date, String str) {
        this(trustAnchor, certPathDefaultConstraints, date, str);
    }
    ...
    public void check(Certificate certificate, Collection<String> collection) throws CertPathValidatorException {
        ...
        try {
            ...
            CertPathConstraintsParameters certPathConstraintsParameters = new CertPathConstraintsParameters(impl, this.variant, this.anchor, this.date);
            if (this.constraints instanceof DisabledAlgorithmConstraints) {
                ((DisabledAlgorithmConstraints) this.constraints).permits(sigAlgName, parameters, (ConstraintsParameters) certPathConstraintsParameters, true);
            } else {
                certPathDefaultConstraints.permits(sigAlgName, parameters, (ConstraintsParameters) certPathConstraintsParameters, true);
                if (!this.constraints.permits(setNoneOf, publicKey)) {
                    throw new CertPathValidatorException("Algorithm constraints check failed on key " + publicKey.getAlgorithm() + " with size of " + KeyUtil.getKeySize(publicKey) + ControllerParameter.PARAM_CLASS_BITS, null, null, -1, CertPathValidatorException.BasicReason.ALGORITHM_CONSTRAINED);
                }
            }
    ...
    static void check(PublicKey publicKey, AlgorithmId algorithmId, String str, TrustAnchor trustAnchor) throws CertPathValidatorException {
        certPathDefaultConstraints.permits(algorithmId.getName(), algorithmId.getParameters(), (ConstraintsParameters) new CertPathConstraintsParameters(publicKey, str, trustAnchor), true);
    }
}

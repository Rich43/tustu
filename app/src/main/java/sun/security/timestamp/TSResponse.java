...
/* loaded from: rt.jar:sun/security/timestamp/TSResponse.class */
public class TSResponse {
    ...
    TSResponse(byte[] bArr) throws IOException {
        parse(bArr);
    }
    ...
    private void parse(byte[] bArr) throws IOException {
        ...
        if (derValue2.data.available() > 0 && ((byte) derValue2.data.peekByte()) == 48) {
            ...
            for (int i2 = 0; i2 < sequence.length; i2++) {
                this.statusString[i2] = sequence[i2].getUTF8String();
                if (debug != null) {
                    debug.println("timestamp response: statusString=" + this.statusString[i2]);
                }
            }
        }
    ...
    /* loaded from: rt.jar:sun/security/timestamp/TSResponse$TimestampException.class */

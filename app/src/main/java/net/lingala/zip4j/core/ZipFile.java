...
/* loaded from: zip4j_1.3.1.jar:net/lingala/zip4j/core/ZipFile.class */
public class ZipFile {
    ...
    public void createZipFile(File sourceFile, ZipParameters parameters) throws ZipException {
        ...
        createZipFile(sourceFileList, parameters, false, -1L);
    }
    ...
    private void readZipInfo() throws ZipException {
        ...
        try {
            try {
                raf = new RandomAccessFile(new File(this.file), InternalZipConstants.READ_MODE);
                if (this.zipModel == null) {
    ...
    public void extractFile(FileHeader fileHeader, String destPath, UnzipParameters unzipParameters) throws ZipException {
        extractFile(fileHeader, destPath, unzipParameters, (String) null);
    }
    ...
    public void extractFile(String fileName, String destPath, UnzipParameters unzipParameters) throws ZipException {
        extractFile(fileName, destPath, unzipParameters, (String) null);
    }
    ...
    public void setPassword(char[] password) throws ZipException {
        ...
        for (int i2 = 0; i2 < this.zipModel.getCentralDirectory().getFileHeaders().size(); i2++) {
            if (this.zipModel.getCentralDirectory().getFileHeaders().get(i2) != null && ((FileHeader) this.zipModel.getCentralDirectory().getFileHeaders().get(i2)).isEncrypted()) {
                ((FileHeader) this.zipModel.getCentralDirectory().getFileHeaders().get(i2)).setPassword(password);
            }
        }
    }
    ...
    public boolean isEncrypted() throws ZipException {
        ...
        int i2 = 0;
        while (true) {
            if (i2 >= fileHeaderList.size()) {
                break;
            }
            FileHeader fileHeader = (FileHeader) fileHeaderList.get(i2);

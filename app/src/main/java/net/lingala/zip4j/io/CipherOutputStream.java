...
/* loaded from: zip4j_1.3.1.jar:net/lingala/zip4j/io/CipherOutputStream.class */
public class CipherOutputStream extends BaseOutputStream {
    ...
    public void putNextEntry(File file, ZipParameters zipParameters) throws ZipException {
        ...
        try {
            ...
            if (!zipParameters.isSourceExternalStream()) {
                ...
                if (this.zipParameters.getFileNameInZip().endsWith("/") || this.zipParameters.getFileNameInZip().endsWith(FXMLLoader.ESCAPE_PREFIX)) {
                    this.zipParameters.setEncryptFiles(false);
                    this.zipParameters.setEncryptionMethod(-1);
                    this.zipParameters.setCompressionMethod(0);
                }
            ...
            if (this.zipParameters.isEncryptFiles()) {
                ...
                if (this.encrypter != null) {
                    if (zipParameters.getEncryptionMethod() == 0) {
                        ...
                        this.totalBytesWritten += saltBytes.length + passwordVerifier.length;
        ...
        if (this.zipParameters.isEncryptFiles() && this.zipParameters.getEncryptionMethod() == 99) {
            if (this.pendingBufferLength != 0) {
                if (len >= 16 - this.pendingBufferLength) {
                    System.arraycopy(b2, off, this.pendingBuffer, this.pendingBufferLength, 16 - this.pendingBufferLength);
                    encryptAndWrite(this.pendingBuffer, 0, this.pendingBuffer.length);
                    off = 16 - this.pendingBufferLength;
                    len -= off;
                    this.pendingBufferLength = 0;
                } else {
                    System.arraycopy(b2, off, this.pendingBuffer, this.pendingBufferLength, len);
                    this.pendingBufferLength += len;
                    return;
                }
            }
            if (len != 0 && len % 16 != 0) {
                System.arraycopy(b2, (len + off) - (len % 16), this.pendingBuffer, 0, len % 16);
                this.pendingBufferLength = len % 16;
                len -= this.pendingBufferLength;
            }
        }
        if (len != 0) {
            encryptAndWrite(b2, off, len);
        }
    }

    private void encryptAndWrite(byte[] b2, int off, int len) throws IOException {
        if (this.encrypter != null) {
            try {
    ...
    private void createFileHeader() throws ZipException {
        ...
        if (this.zipParameters.isSourceExternalStream()) {
            this.fileHeader.setDirectory(fileName.endsWith("/") || fileName.endsWith(FXMLLoader.ESCAPE_PREFIX));
        } else {
        ...
        if (this.fileHeader.isDirectory()) {
            ...
            if (this.zipParameters.getCompressionMethod() == 0) {
                if (this.zipParameters.getEncryptionMethod() == 0) {
                    ...
                    this.fileHeader.setCompressedSize(fileSize + saltLength + 10 + 2);
                } else {
                    this.fileHeader.setCompressedSize(0L);
                }
            } else {

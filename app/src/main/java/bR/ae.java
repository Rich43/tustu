package br;

import G.C0113cs;
import G.InterfaceC0109co;
import com.efiAnalytics.ui.cA;
import java.util.ArrayList;
import java.util.Iterator;

/* loaded from: TunerStudioMS.jar:br/ae.class */
class ae implements InterfaceC0109co {

    /* renamed from: a, reason: collision with root package name */
    ArrayList f8413a = new ArrayList();

    /* renamed from: b, reason: collision with root package name */
    final /* synthetic */ P f8414b;

    ae(P p2, G.R r2, String str) {
        this.f8414b = p2;
        try {
            C0113cs.a().a(r2.c(), str, this);
        } catch (V.a e2) {
            bH.C.a("Paint updates disabled for Component. " + e2.getMessage());
            e2.printStackTrace();
        }
    }

    @Override // G.InterfaceC0109co
    public void setCurrentOutputChannelValue(String str, double d2) {
        Iterator it = this.f8413a.iterator();
        while (it.hasNext()) {
            ((cA) it.next()).a();
        }
    }

    public void a(cA cAVar) {
        this.f8413a.add(cAVar);
    }
}

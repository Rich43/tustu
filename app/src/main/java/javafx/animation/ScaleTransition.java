...
/* loaded from: jfxrt.jar:javafx/animation/ScaleTransition.class */
public final class ScaleTransition extends Transition {
    ...
    public final ObjectProperty<Duration> durationProperty() {
        if (this.duration == null) {
            this.duration = new ObjectPropertyBase<Duration>(DEFAULT_DURATION) { // from class: javafx.animation.ScaleTransition.1
                ...
                public void invalidated() {
                    try {
                        ...
                        set(ScaleTransition.this.getCycleDuration());
                        throw e2;
                    }
                }
    ...
    public void interpolate(double frac) {
        if (!Double.isNaN(this.startX)) {
            this.cachedNode.setScaleX(this.startX + (frac * this.deltaX));
        }
        if (!Double.isNaN(this.startY)) {
            this.cachedNode.setScaleY(this.startY + (frac * this.deltaY));
        }
        if (!Double.isNaN(this.startZ)) {
            this.cachedNode.setScaleZ(this.startZ + (frac * this.deltaZ));
        }
    }
    ...
    void impl_sync(boolean forceSync) {
        ...
        if (forceSync || this.cachedNode == null) {
            ...
            if (Double.isNaN(_fromX) && Double.isNaN(_toX) && Math.abs(_byX) < 1.0E-12d) {
                ...
                this.startX = !Double.isNaN(_fromX) ? _fromX : this.cachedNode.getScaleX();
                this.deltaX = !Double.isNaN(_toX) ? _toX - this.startX : getByX();
            }
            if (Double.isNaN(_fromY) && Double.isNaN(_toY) && Math.abs(_byY) < 1.0E-12d) {
                this.startY = Double.NaN;
            } else {
                this.startY = !Double.isNaN(_fromY) ? _fromY : this.cachedNode.getScaleY();
            ...
            if (Double.isNaN(_fromZ) && Double.isNaN(_toZ) && Math.abs(_byZ) < 1.0E-12d) {
                ...
                this.startZ = !Double.isNaN(_fromZ) ? _fromZ : this.cachedNode.getScaleZ();

...
/* loaded from: jfxrt.jar:com/sun/prism/d3d/D3DTexture.class */
class D3DTexture extends BaseTexture<D3DTextureResource> implements D3DContextSource {
    ...
    public void update(MediaFrame frame, boolean skipFlush) {
        ...
        if (frame.getPixelFormat() == PixelFormat.MULTI_YCbCr_420) {
            throw new IllegalArgumentException("Unsupported format " + ((Object) frame.getPixelFormat()));
        }
        frame.holdFrame();
        ...
        if (targetFormat.getDataType() == PixelFormat.DataType.INT) {
            result = D3DResourceFactory.nUpdateTextureI(ctx.getContextHandle(), getNativeSourceHandle(), pixels.asIntBuffer(), null, 0, 0, 0, 0, frame.getEncodedWidth(), frame.getEncodedHeight(), frame.strideForPlane(0));
        } else {
            result = D3DResourceFactory.nUpdateTextureB(ctx.getContextHandle(), getNativeSourceHandle(), pixels, null, targetFormat.ordinal(), 0, 0, 0, 0, frame.getEncodedWidth(), frame.getEncodedHeight(), frame.strideForPlane(0));
        }
    ...
    public void update(Buffer pixels, PixelFormat format, int dstx, int dsty, int srcx, int srcy, int srcw, int srch, int srcscan, boolean skipFlush) {
        checkUpdateParams(pixels, format, dstx, dsty, srcx, srcy, srcw, srch, srcscan);
        if (!skipFlush) {
        ...
        update(pixels, format, contentX + dstx, contentY + dsty, srcx, srcy, srcw, srch, srcscan);
        switch (getWrapMode()) {
            case CLAMP_TO_EDGE_SIMULATED:
                boolean copyR = contentW < texWidth && dstx + srcw == contentW;
                boolean copyL = contentH < texHeight && dsty + srch == contentH;
                if (copyR) {
                    update(pixels, format, contentX + contentW, contentY + dsty, (srcx + srcw) - 1, srcy, 1, srch, srcscan);
                }
                if (copyL) {
                    update(pixels, format, contentX + dstx, contentY + contentH, srcx, (srcy + srch) - 1, srcw, 1, srcscan);
                    if (copyR) {
                        update(pixels, format, contentX + contentW, contentY + contentH, (srcx + srcw) - 1, (srcy + srch) - 1, 1, 1, srcscan);
                        break;
                    }
                }
                break;
            case REPEAT_SIMULATED:
                ...
                if (repeatL) {
                    update(pixels, format, contentX + contentW, contentY + dsty, srcx, srcy, 1, srch, srcscan);
                }
                if (repeatT) {
                    update(pixels, format, contentX + dstx, contentY + contentH, srcx, srcy, srcw, 1, srcscan);
                    if (repeatL) {
                        update(pixels, format, contentX + contentW, contentY + contentH, srcx, srcy, 1, 1, srcscan);
                        break;
                    }
                }
                break;
    ...
    public void update(Buffer pixels, PixelFormat format, int dstx, int dsty, int srcx, int srcy, int srcw, int srch, int srcscan) {
        ...
        if (format.getDataType() == PixelFormat.DataType.INT) {
            ...
            res = D3DResourceFactory.nUpdateTextureI(ctx.getContextHandle(), getNativeSourceHandle(), buf, arr, dstx, dsty, srcx, srcy, srcw, srch, srcscan);
        } else if (format.getDataType() == PixelFormat.DataType.FLOAT) {
            FloatBuffer buf2 = (FloatBuffer) pixels;
            float[] arr2 = buf2.hasArray() ? buf2.array() : null;
            res = D3DResourceFactory.nUpdateTextureF(ctx.getContextHandle(), getNativeSourceHandle(), buf2, arr2, dstx, dsty, srcx, srcy, srcw, srch, srcscan);
        } else {
            ByteBuffer buf3 = (ByteBuffer) pixels;
            buf3.rewind();
            byte[] arr3 = buf3.hasArray() ? buf3.array() : null;
            res = D3DResourceFactory.nUpdateTextureB(ctx.getContextHandle(), getNativeSourceHandle(), buf3, arr3, format.ordinal(), dstx, dsty, srcx, srcy, srcw, srch, srcscan);
        }

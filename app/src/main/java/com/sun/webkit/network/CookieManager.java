...
/* loaded from: jfxrt.jar:com/sun/webkit/network/CookieManager.class */
public final class CookieManager extends CookieHandler {
    ...
    private String get(URI uri) {
        ...
        String host2 = canonicalize(host);
        ...
        synchronized (this.store) {
            cookieList = this.store.get(host2, uri.getPath(), secureProtocol, httpApi);
        }
    ...
    private void put(URI uri, Cookie cookie) {
        ...
        if (cookie.getDomain().length() > 0) {
            if (!Cookie.domainMatches(host2, cookie.getDomain())) {

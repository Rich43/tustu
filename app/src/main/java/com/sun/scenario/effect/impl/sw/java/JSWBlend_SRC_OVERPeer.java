...
/* loaded from: jfxrt.jar:com/sun/scenario/effect/impl/sw/java/JSWBlend_SRC_OVERPeer.class */
public class JSWBlend_SRC_OVERPeer extends JSWEffectPeer {
    public JSWBlend_SRC_OVERPeer(FilterContext fctx, Renderer r2, String uniqueName) {
    ...
    public ImageData filter(Effect effect, RenderState rstate, BaseTransform transform, Rectangle outputClip, ImageData... inputs) {
        ...
        for (int dy = 0; dy < 0 + dsth; dy++) {
            ...
            for (int dx = 0; dx < 0 + dstw; dx++) {
                ...
                float sample_res_w2 = (topImg_tmp >>> 24) / 255.0f;
                float top_x = sample_res_x2 * opacity;
                float top_y = sample_res_y2 * opacity;
                float top_z = sample_res_z2 * opacity;
                float top_w = sample_res_w2 * opacity;
                float blend_src_over_res_x = top_x + (sample_res_x * (1.0f - top_w));
                float blend_src_over_res_y = top_y + (sample_res_y * (1.0f - top_w));
                float blend_src_over_res_z = top_z + (sample_res_z * (1.0f - top_w));
                float blend_src_over_res_w = top_w + (sample_res_w * (1.0f - top_w));

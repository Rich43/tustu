...
/* loaded from: jfxrt.jar:com/sun/scenario/effect/impl/sw/java/JSWBlend_SRC_ATOPPeer.class */
public class JSWBlend_SRC_ATOPPeer extends JSWEffectPeer {
    public JSWBlend_SRC_ATOPPeer(FilterContext fctx, Renderer r2, String uniqueName) {
    ...
    public ImageData filter(Effect effect, RenderState rstate, BaseTransform transform, Rectangle outputClip, ImageData... inputs) {
        ...
        for (int dy = 0; dy < 0 + dsth; dy++) {
            ...
            for (int dx = 0; dx < 0 + dstw; dx++) {
                ...
                float blend_src_atop_res_x = (top_x * sample_res_w) + (sample_res_x * (1.0f - top_w));
                float blend_src_atop_res_y = (top_y * sample_res_w) + (sample_res_y * (1.0f - top_w));
                float blend_src_atop_res_z = (top_z * sample_res_w) + (sample_res_z * (1.0f - top_w));
                float blend_src_atop_res_w = (top_w * sample_res_w) + (sample_res_w * (1.0f - top_w));
                float color_x = blend_src_atop_res_x;
                float color_y = blend_src_atop_res_y;
                float color_z = blend_src_atop_res_z;
                float color_w = blend_src_atop_res_w;
                if (color_w < 0.0f) {
                    color_w = 0.0f;

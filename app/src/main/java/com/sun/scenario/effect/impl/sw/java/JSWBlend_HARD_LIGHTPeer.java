...
/* loaded from: jfxrt.jar:com/sun/scenario/effect/impl/sw/java/JSWBlend_HARD_LIGHTPeer.class */
public class JSWBlend_HARD_LIGHTPeer extends JSWEffectPeer {
    public JSWBlend_HARD_LIGHTPeer(FilterContext fctx, Renderer r2, String uniqueName) {
    ...
    public ImageData filter(Effect effect, RenderState rstate, BaseTransform transform, Rectangle outputClip, ImageData... inputs) {
        ...
        for (int dy = 0; dy < 0 + dsth; dy++) {
            ...
            for (int dx = 0; dx < 0 + dstw; dx++) {
                ...
                float halftopa = 0.5f * top_w;
                if (top_x > halftopa) {
                    res_x = (top_x + (sample_res_w * (top_x - top_w))) - (sample_res_x * (((2.0f * top_x) - top_w) - 1.0f));
                } else {
                    res_x = (2.0f * sample_res_x * top_x) + (sample_res_x * (1.0f - top_w)) + (top_x * (1.0f - sample_res_w));
                }
                if (top_y > halftopa) {
                    res_y = (top_y + (sample_res_w * (top_y - top_w))) - (sample_res_y * (((2.0f * top_y) - top_w) - 1.0f));
                } else {
                    res_y = (2.0f * sample_res_y * top_y) + (sample_res_y * (1.0f - top_w)) + (top_y * (1.0f - sample_res_w));
                }
                if (top_z > halftopa) {
                    res_z = (top_z + (sample_res_w * (top_z - top_w))) - (sample_res_z * (((2.0f * top_z) - top_w) - 1.0f));
                } else {
                    res_z = (2.0f * sample_res_z * top_z) + (sample_res_z * (1.0f - top_w)) + (top_z * (1.0f - sample_res_w));
                }
                float blend_hard_light_res_x = res_x;
                float blend_hard_light_res_y = res_y;
                float blend_hard_light_res_z = res_z;
                float color_x = blend_hard_light_res_x;
                float color_y = blend_hard_light_res_y;
                float color_z = blend_hard_light_res_z;
                float color_w = res_w;
                if (color_w < 0.0f) {
                    color_w = 0.0f;

...
/* loaded from: jfxrt.jar:com/sun/scenario/effect/impl/sw/java/JSWBlend_COLOR_BURNPeer.class */
public class JSWBlend_COLOR_BURNPeer extends JSWEffectPeer {
    public JSWBlend_COLOR_BURNPeer(FilterContext fctx, Renderer r2, String uniqueName) {
    ...
    public ImageData filter(Effect effect, RenderState rstate, BaseTransform transform, Rectangle outputClip, ImageData... inputs) {
        ...
        for (int dy = 0; dy < 0 + dsth; dy++) {
            ...
            for (int dx = 0; dx < 0 + dstw; dx++) {
                ...
                float res_x = ((1.0f - top_w) * sample_res_x) + ((1.0f - sample_res_w) * top_x);
                float res_y = ((1.0f - top_w) * sample_res_y) + ((1.0f - sample_res_w) * top_y);
                float res_z = ((1.0f - top_w) * sample_res_z) + ((1.0f - sample_res_w) * top_z);
                float proda = sample_res_w * top_w;
                float topa2 = top_w * top_w;
                if (sample_res_w == sample_res_x) {
                    tmp_x = proda;
                } else if (top_x != 0.0f) {
                    float tmp_x2 = (topa2 * (sample_res_w - sample_res_x)) / top_x;
                    if (tmp_x2 >= proda) {
                        tmp_x = 0.0f;
                    } else {
                        tmp_x = proda - tmp_x2;
                ...
                if (sample_res_w == sample_res_y) {
                    tmp_y = proda;
                } else if (top_y != 0.0f) {
                    float tmp_y2 = (topa2 * (sample_res_w - sample_res_y)) / top_y;
                    if (tmp_y2 >= proda) {
                        tmp_y = 0.0f;
                    } else {
                        tmp_y = proda - tmp_y2;
                ...
                if (sample_res_w == sample_res_z) {
                    tmp_z = proda;
                } else if (top_z != 0.0f) {
                    float tmp_z2 = (topa2 * (sample_res_w - sample_res_z)) / top_z;
                    if (tmp_z2 >= proda) {
                        tmp_z = 0.0f;
                    } else {
                        tmp_z = proda - tmp_z2;
                ...
                float color_x = res_x + tmp_x;
                float color_y = res_y + tmp_y;
                float color_z = res_z + tmp_z;
                float color_w = res_w;
                if (color_w < 0.0f) {
                    color_w = 0.0f;

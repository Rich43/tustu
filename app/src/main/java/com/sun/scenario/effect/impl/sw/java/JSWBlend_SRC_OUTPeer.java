...
/* loaded from: jfxrt.jar:com/sun/scenario/effect/impl/sw/java/JSWBlend_SRC_OUTPeer.class */
public class JSWBlend_SRC_OUTPeer extends JSWEffectPeer {
    public JSWBlend_SRC_OUTPeer(FilterContext fctx, Renderer r2, String uniqueName) {
    ...
    public ImageData filter(Effect effect, RenderState rstate, BaseTransform transform, Rectangle outputClip, ImageData... inputs) {
        ...
        for (int dy = 0; dy < 0 + dsth; dy++) {
            ...
            for (int dx = 0; dx < 0 + dstw; dx++) {
                ...
                float blend_src_out_res_x = top_x * (1.0f - sample_res_w);
                float blend_src_out_res_y = top_y * (1.0f - sample_res_w);
                float blend_src_out_res_z = top_z * (1.0f - sample_res_w);
                float blend_src_out_res_w = top_w * (1.0f - sample_res_w);

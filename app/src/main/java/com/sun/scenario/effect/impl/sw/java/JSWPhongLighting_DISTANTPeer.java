...
/* loaded from: jfxrt.jar:com/sun/scenario/effect/impl/sw/java/JSWPhongLighting_DISTANTPeer.class */
public class JSWPhongLighting_DISTANTPeer extends JSWEffectPeer {
    private FloatBuffer kvals;

    public JSWPhongLighting_DISTANTPeer(FilterContext fctx, Renderer r2, String uniqueName) {
        ...
        for (int dy = 0; dy < 0 + dsth; dy++) {
            ...
            for (int dx = 0; dx < 0 + dstw; dx++) {
                ...
                for (int i3 = 0; i3 < 8; i3++) {
                    ...
                    int bumpImg_tmp = i2;
                    float sample_res_w2 = (bumpImg_tmp >>> 24) / 255.0f;
                ...
                float x_tmp_x2 = normalizedLightPosition_x + 0.0f;
                float x_tmp_y2 = normalizedLightPosition_y + 0.0f;
                float x_tmp_z = normalizedLightPosition_z + 1.0f;
                ...
                float dot_res = (normalize_res_x * normalizedLightPosition_x) + (normalize_res_y * normalizedLightPosition_y) + (normalize_res_z * normalizedLightPosition_z);
                ...
                float pow_res = (float) Math.pow((normalize_res_x * normalize_res_x2) + (normalize_res_y * normalize_res_y2) + (normalize_res_z * normalize_res_z2), specularExponent);

...
/* loaded from: jfxrt.jar:com/sun/scenario/effect/impl/sw/java/JSWBlend_LIGHTENPeer.class */
public class JSWBlend_LIGHTENPeer extends JSWEffectPeer {
    public JSWBlend_LIGHTENPeer(FilterContext fctx, Renderer r2, String uniqueName) {
    ...
    public ImageData filter(Effect effect, RenderState rstate, BaseTransform transform, Rectangle outputClip, ImageData... inputs) {
        ...
        for (int dy = 0; dy < 0 + dsth; dy++) {
            ...
            for (int dx = 0; dx < 0 + dstw; dx++) {
                ...
                float res_x = (sample_res_x + top_x) - min_res_x;
                float res_y = (sample_res_y + top_y) - min_res_y;
                float res_z = (sample_res_z + top_z) - min_res_z;
                float color_x = res_x;
                float color_y = res_y;
                float color_z = res_z;
                float color_w = res_w;

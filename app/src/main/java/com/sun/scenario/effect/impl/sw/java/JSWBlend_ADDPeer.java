...
/* loaded from: jfxrt.jar:com/sun/scenario/effect/impl/sw/java/JSWBlend_ADDPeer.class */
public class JSWBlend_ADDPeer extends JSWEffectPeer {
    public JSWBlend_ADDPeer(FilterContext fctx, Renderer r2, String uniqueName) {
    ...
    public ImageData filter(Effect effect, RenderState rstate, BaseTransform transform, Rectangle outputClip, ImageData... inputs) {
        ...
        for (int dy = 0; dy < 0 + dsth; dy++) {
            ...
            for (int dx = 0; dx < 0 + dstw; dx++) {
                ...
                float x_tmp_x = ((sample_res_x * top_w) + (top_x * sample_res_w)) - (top_w * sample_res_w);
                float x_tmp_y = ((sample_res_y * top_w) + (top_y * sample_res_w)) - (top_w * sample_res_w);
                float x_tmp_z = ((sample_res_z * top_w) + (top_z * sample_res_w)) - (top_w * sample_res_w);
                float x_tmp_w = ((sample_res_w * top_w) + (top_w * sample_res_w)) - (top_w * sample_res_w);
                float max_res_x = x_tmp_x > 0.0f ? x_tmp_x : 0.0f;
                float max_res_y = x_tmp_y > 0.0f ? x_tmp_y : 0.0f;
                float max_res_z = x_tmp_z > 0.0f ? x_tmp_z : 0.0f;
                float max_res_w = x_tmp_w > 0.0f ? x_tmp_w : 0.0f;
                float blend_add_res_x = (sample_res_x + top_x) - max_res_x;
                float blend_add_res_y = (sample_res_y + top_y) - max_res_y;
                float blend_add_res_z = (sample_res_z + top_z) - max_res_z;
                float blend_add_res_w = (sample_res_w + top_w) - max_res_w;
                float color_x = blend_add_res_x;
                float color_y = blend_add_res_y;
                float color_z = blend_add_res_z;
                float color_w = blend_add_res_w;
                if (color_w < 0.0f) {
                    color_w = 0.0f;

...
/* loaded from: jfxrt.jar:com/sun/scenario/effect/impl/sw/java/JSWBlend_SOFT_LIGHTPeer.class */
public class JSWBlend_SOFT_LIGHTPeer extends JSWEffectPeer {
    public JSWBlend_SOFT_LIGHTPeer(FilterContext fctx, Renderer r2, String uniqueName) {
    ...
    public ImageData filter(Effect effect, RenderState rstate, BaseTransform transform, Rectangle outputClip, ImageData... inputs) {
        ...
        for (int dy = 0; dy < 0 + dsth; dy++) {
            ...
            for (int dx = 0; dx < 0 + dstw; dx++) {
                ...
                float bot_np_x = sample_res_x / sample_res_w;
                float bot_np_y = sample_res_y / sample_res_w;
                float bot_np_z = sample_res_z / sample_res_w;
                float top_np_x = top_x / top_w;
                float top_np_y = top_y / top_w;
                float top_np_z = top_z / top_w;
                float sqrt_res = (float) Math.sqrt(bot_np_x);
                if (bot_np_x <= 0.25f) {
                    d2 = ((((16.0f * bot_np_x) - 12.0f) * bot_np_x) + 4.0f) * bot_np_x;
                } else {
                    d2 = sqrt_res;
                }
                float dofx_res = d2;
                float sqrt_res2 = (float) Math.sqrt(bot_np_y);
                if (bot_np_y <= 0.25f) {
                    d3 = ((((16.0f * bot_np_y) - 12.0f) * bot_np_y) + 4.0f) * bot_np_y;
                } else {
                    d3 = sqrt_res2;
                }
                float dofx_res2 = d3;
                float sqrt_res3 = (float) Math.sqrt(bot_np_z);
                if (bot_np_z <= 0.25f) {
                    d4 = ((((16.0f * bot_np_z) - 12.0f) * bot_np_z) + 4.0f) * bot_np_z;
                } else {
                    d4 = sqrt_res3;
                }
                float dofx_res3 = d4;
                if (sample_res_w == 0.0f) {
                    ...
                    res_x = sample_res_x;
                } else if (top_np_x > 0.5f) {
                    res_x = sample_res_x + ((1.0f - sample_res_w) * top_x) + (((2.0f * top_x) - top_w) * ((sample_res_w * dofx_res) - sample_res_x));
                } else {
                    res_x = (sample_res_x + ((1.0f - sample_res_w) * top_x)) - (((top_w * sample_res_x) * (1.0f - (2.0f * top_np_x))) * (1.0f - bot_np_x));
                }
                if (sample_res_w == 0.0f) {
                    res_y = top_y;
                } else if (top_w == 0.0f) {
                    res_y = sample_res_y;
                } else if (top_np_y > 0.5f) {
                    res_y = sample_res_y + ((1.0f - sample_res_w) * top_y) + (((2.0f * top_y) - top_w) * ((sample_res_w * dofx_res2) - sample_res_y));
                } else {
                    res_y = (sample_res_y + ((1.0f - sample_res_w) * top_y)) - (((top_w * sample_res_y) * (1.0f - (2.0f * top_np_y))) * (1.0f - bot_np_y));
                }
                if (sample_res_w == 0.0f) {
                    res_z = top_z;
                } else if (top_w == 0.0f) {
                    res_z = sample_res_z;
                } else if (top_np_z > 0.5f) {
                    res_z = sample_res_z + ((1.0f - sample_res_w) * top_z) + (((2.0f * top_z) - top_w) * ((sample_res_w * dofx_res3) - sample_res_z));
                } else {
                    res_z = (sample_res_z + ((1.0f - sample_res_w) * top_z)) - (((top_w * sample_res_z) * (1.0f - (2.0f * top_np_z))) * (1.0f - bot_np_z));
                }
                float blend_soft_light_res_x = res_x;
                float blend_soft_light_res_y = res_y;
                float blend_soft_light_res_z = res_z;
                float color_x = blend_soft_light_res_x;
                float color_y = blend_soft_light_res_y;
                float color_z = blend_soft_light_res_z;
                float color_w = res_w;
                if (color_w < 0.0f) {
                    color_w = 0.0f;

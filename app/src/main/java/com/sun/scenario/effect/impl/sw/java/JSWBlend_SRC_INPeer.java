...
/* loaded from: jfxrt.jar:com/sun/scenario/effect/impl/sw/java/JSWBlend_SRC_INPeer.class */
public class JSWBlend_SRC_INPeer extends JSWEffectPeer {
    public JSWBlend_SRC_INPeer(FilterContext fctx, Renderer r2, String uniqueName) {
    ...
    public ImageData filter(Effect effect, RenderState rstate, BaseTransform transform, Rectangle outputClip, ImageData... inputs) {
        ...
        for (int dy = 0; dy < 0 + dsth; dy++) {
            ...
            for (int dx = 0; dx < 0 + dstw; dx++) {
                ...
                float f4 = ((botImg_tmp >> 16) & 255) / 255.0f;
                float f5 = ((botImg_tmp >> 8) & 255) / 255.0f;
                float f6 = (botImg_tmp & 255) / 255.0f;
                ...
                float sample_res_x = ((topImg_tmp >> 16) & 255) / 255.0f;
                float sample_res_y = ((topImg_tmp >> 8) & 255) / 255.0f;
                float sample_res_z = (topImg_tmp & 255) / 255.0f;
                ...
                float blend_src_in_res_x = top_x * sample_res_w;
                float blend_src_in_res_y = top_y * sample_res_w;
                float blend_src_in_res_z = top_z * sample_res_w;
                float blend_src_in_res_w = top_w * sample_res_w;
                float color_x = blend_src_in_res_x;
                float color_y = blend_src_in_res_y;
                float color_z = blend_src_in_res_z;
                float color_w = blend_src_in_res_w;
                if (color_w < 0.0f) {
                    color_w = 0.0f;

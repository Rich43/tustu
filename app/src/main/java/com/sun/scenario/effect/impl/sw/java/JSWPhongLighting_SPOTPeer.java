...
/* loaded from: jfxrt.jar:com/sun/scenario/effect/impl/sw/java/JSWPhongLighting_SPOTPeer.class */
public class JSWPhongLighting_SPOTPeer extends JSWEffectPeer {
    private FloatBuffer kvals;

    public JSWPhongLighting_SPOTPeer(FilterContext fctx, Renderer r2, String uniqueName) {
    ...
    public ImageData filter(Effect effect, RenderState rstate, BaseTransform transform, Rectangle outputClip, ImageData... inputs) {
        ...
        float normalizedLightDirection_x = normalizedLightDirection_arr[0];
        float normalizedLightDirection_y = normalizedLightDirection_arr[1];
        float normalizedLightDirection_z = normalizedLightDirection_arr[2];
        ...
        for (int dy = 0; dy < 0 + dsth; dy++) {
            ...
            for (int dx = 0; dx < 0 + dstw; dx++) {
                ...
                float dot_res = (normalize_res_x2 * normalizedLightDirection_x) + (normalize_res_y2 * normalizedLightDirection_y) + (normalize_res_z2 * normalizedLightDirection_z);
                if (dot_res < 0.0f) {
                    f2 = dot_res;
                } else {
                    f2 = 0.0f;
                ...
                float min_res = f2;
                float x_tmp = -min_res;
                float pow_res = (float) Math.pow(x_tmp, lightSpecularExponent);
                ...
                float dot_res2 = (normalize_res_x * normalize_res_x2) + (normalize_res_y * normalize_res_y2) + (normalize_res_z * normalize_res_z2);
                float D_x = diffuseConstant * dot_res2 * Lrgb_x;
                float D_y = diffuseConstant * dot_res2 * Lrgb_y;
                float D_z = diffuseConstant * dot_res2 * Lrgb_z;
                if (D_x < 0.0f) {
                    f3 = 0.0f;
                } else if (D_x <= 1.0f) {
                    f3 = D_x;
                ...
                float clamp_res_z = f5;
                float pow_res2 = (float) Math.pow((normalize_res_x * normalize_res_x3) + (normalize_res_y * normalize_res_y3) + (normalize_res_z * normalize_res_z3), specularExponent);
                float S_x = specularConstant * pow_res2 * Lrgb_x;
                float S_y = specularConstant * pow_res2 * Lrgb_y;
                float S_z = specularConstant * pow_res2 * Lrgb_z;
                if (S_x > S_y) {
                    f6 = S_x;
                } else {
                    f6 = S_y;

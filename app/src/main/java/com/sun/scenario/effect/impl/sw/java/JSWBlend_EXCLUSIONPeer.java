...
/* loaded from: jfxrt.jar:com/sun/scenario/effect/impl/sw/java/JSWBlend_EXCLUSIONPeer.class */
public class JSWBlend_EXCLUSIONPeer extends JSWEffectPeer {
    public JSWBlend_EXCLUSIONPeer(FilterContext fctx, Renderer r2, String uniqueName) {
    ...
    public ImageData filter(Effect effect, RenderState rstate, BaseTransform transform, Rectangle outputClip, ImageData... inputs) {
        ...
        for (int dy = 0; dy < 0 + dsth; dy++) {
            ...
            for (int dx = 0; dx < 0 + dstw; dx++) {
                ...
                float res_x = (top_x + sample_res_x) - ((2.0f * top_x) * sample_res_x);
                float res_y = (top_y + sample_res_y) - ((2.0f * top_y) * sample_res_y);
                float res_z = (top_z + sample_res_z) - ((2.0f * top_z) * sample_res_z);
                float color_x = res_x;
                float color_y = res_y;
                float color_z = res_z;
                float color_w = res_w;

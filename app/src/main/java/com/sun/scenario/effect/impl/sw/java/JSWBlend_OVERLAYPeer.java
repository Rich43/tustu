...
/* loaded from: jfxrt.jar:com/sun/scenario/effect/impl/sw/java/JSWBlend_OVERLAYPeer.class */
public class JSWBlend_OVERLAYPeer extends JSWEffectPeer {
    public JSWBlend_OVERLAYPeer(FilterContext fctx, Renderer r2, String uniqueName) {
    ...
    public ImageData filter(Effect effect, RenderState rstate, BaseTransform transform, Rectangle outputClip, ImageData... inputs) {
        ...
        for (int dy = 0; dy < 0 + dsth; dy++) {
            ...
            for (int dx = 0; dx < 0 + dstw; dx++) {
                ...
                float x_tmp_x = sample_res_x - (sample_res_w * 0.5f);
                float x_tmp_y = sample_res_y - (sample_res_w * 0.5f);
                float x_tmp_z = sample_res_z - (sample_res_w * 0.5f);
                float ceil_res_x = (float) Math.ceil(x_tmp_x);
                float ceil_res_y = (float) Math.ceil(x_tmp_y);
                float ceil_res_z = (float) Math.ceil(x_tmp_z);
                float x_tmp_x2 = sample_res_x - (ceil_res_x * sample_res_w);
                float x_tmp_y2 = sample_res_y - (ceil_res_y * sample_res_w);
                float x_tmp_z2 = sample_res_z - (ceil_res_z * sample_res_w);
                float abs_res_x = Math.abs(x_tmp_x2);
                float abs_res_y = Math.abs(x_tmp_y2);
                float abs_res_z = Math.abs(x_tmp_z2);
                float x_tmp_x3 = top_x - (ceil_res_x * top_w);
                float x_tmp_y3 = top_y - (ceil_res_y * top_w);
                float x_tmp_z3 = top_z - (ceil_res_z * top_w);
                float abs_res_x2 = Math.abs(x_tmp_x3);
                float abs_res_y2 = Math.abs(x_tmp_y3);
                float abs_res_z2 = Math.abs(x_tmp_z3);
                float res_x = ((((2.0f * abs_res_x) + 1.0f) - sample_res_w) * abs_res_x2) + ((1.0f - top_w) * abs_res_x);
                float res_y = ((((2.0f * abs_res_y) + 1.0f) - sample_res_w) * abs_res_y2) + ((1.0f - top_w) * abs_res_y);
                float res_z = ((((2.0f * abs_res_z) + 1.0f) - sample_res_w) * abs_res_z2) + ((1.0f - top_w) * abs_res_z);
                float x_tmp_x4 = res_x - (ceil_res_x * res_w);
                float x_tmp_y4 = res_y - (ceil_res_y * res_w);
                float x_tmp_z4 = res_z - (ceil_res_z * res_w);
                float color_x = Math.abs(x_tmp_x4);
                float color_y = Math.abs(x_tmp_y4);
                float color_z = Math.abs(x_tmp_z4);
                float color_w = res_w;
                if (color_w < 0.0f) {
                    color_w = 0.0f;

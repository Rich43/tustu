...
/* loaded from: jfxrt.jar:com/sun/scenario/effect/impl/sw/java/JSWBlend_MULTIPLYPeer.class */
public class JSWBlend_MULTIPLYPeer extends JSWEffectPeer {
    public JSWBlend_MULTIPLYPeer(FilterContext fctx, Renderer r2, String uniqueName) {
    ...
    public ImageData filter(Effect effect, RenderState rstate, BaseTransform transform, Rectangle outputClip, ImageData... inputs) {
        ...
        for (int dy = 0; dy < 0 + dsth; dy++) {
            ...
            for (int dx = 0; dx < 0 + dstw; dx++) {
                ...
                float sample_res_x2 = ((topImg_tmp >> 16) & 255) / 255.0f;
                float sample_res_y2 = ((topImg_tmp >> 8) & 255) / 255.0f;
                float sample_res_z2 = (topImg_tmp & 255) / 255.0f;
                float top_x = sample_res_x2 * opacity;
                float top_y = sample_res_y2 * opacity;
                float top_z = sample_res_z2 * opacity;
                float top_w = ((topImg_tmp >>> 24) / 255.0f) * opacity;
                float blend_multiply_res_x = (sample_res_x * ((top_x + 1.0f) - top_w)) + (top_x * (1.0f - sample_res_w));
                float blend_multiply_res_y = (sample_res_y * ((top_y + 1.0f) - top_w)) + (top_y * (1.0f - sample_res_w));
                float blend_multiply_res_z = (sample_res_z * ((top_z + 1.0f) - top_w)) + (top_z * (1.0f - sample_res_w));
                float blend_multiply_res_w = (sample_res_w * ((top_w + 1.0f) - top_w)) + (top_w * (1.0f - sample_res_w));
                float color_x = blend_multiply_res_x;
                float color_y = blend_multiply_res_y;
                float color_z = blend_multiply_res_z;
                float color_w = blend_multiply_res_w;
                if (color_w < 0.0f) {
                    color_w = 0.0f;

...
/* loaded from: jfxrt.jar:com/sun/scenario/effect/impl/sw/java/JSWBlend_DIFFERENCEPeer.class */
public class JSWBlend_DIFFERENCEPeer extends JSWEffectPeer {
    public JSWBlend_DIFFERENCEPeer(FilterContext fctx, Renderer r2, String uniqueName) {
    ...
    public ImageData filter(Effect effect, RenderState rstate, BaseTransform transform, Rectangle outputClip, ImageData... inputs) {
        ...
        for (int dy = 0; dy < 0 + dsth; dy++) {
            ...
            for (int dx = 0; dx < 0 + dstw; dx++) {
                ...
                float min_res_x = x_tmp_x < y_tmp_x ? x_tmp_x : y_tmp_x;
                float min_res_y = x_tmp_y < y_tmp_y ? x_tmp_y : y_tmp_y;
                float min_res_z = x_tmp_z < y_tmp_z ? x_tmp_z : y_tmp_z;
                float res_x = (sample_res_x + top_x) - (2.0f * min_res_x);
                float res_y = (sample_res_y + top_y) - (2.0f * min_res_y);
                float res_z = (sample_res_z + top_z) - (2.0f * min_res_z);
                float color_x = res_x;
                float color_y = res_y;
                float color_z = res_z;
                float color_w = res_w;

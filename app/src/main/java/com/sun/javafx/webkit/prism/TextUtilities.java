...
/* loaded from: jfxrt.jar:com/sun/javafx/webkit/prism/TextUtilities.class */
final class TextUtilities {
    ...
    static TextRun createGlyphList(int[] glyphs, float[] advances, float x2, float y2) {
        TextRun run = new TextRun(0, glyphs.length, (byte) 0, true, 0, null, 0, false) { // from class: com.sun.javafx.webkit.prism.TextUtilities.1
            @Override // com.sun.javafx.text.TextRun, com.sun.javafx.scene.text.GlyphList
            public RectBounds getLineBounds() {
                return new RectBounds();
            }
        };
        run.shape(glyphs.length, glyphs, advances);
        run.setLocation(x2, y2);
        return run;
    }
}

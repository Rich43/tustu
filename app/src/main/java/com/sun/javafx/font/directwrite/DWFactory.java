...
/* loaded from: jfxrt.jar:com/sun/javafx/font/directwrite/DWFactory.class */
public class DWFactory extends PrismFontFactory {
    ...
    protected PrismFontFile createFontFile(String name, String filename, int fIndex, boolean register, boolean embedded, boolean copy, boolean tracked) throws Exception {
        return new DWFontFile(name, filename, fIndex, register, embedded, copy, tracked);
    }
    ...
    private static void checkThread() {
        ...
        if (d2dThread != current) {
            throw new IllegalStateException("This operation is not permitted on the current thread [" + current.getName() + "]");
        }
    }

...
/* loaded from: jfxrt.jar:com/sun/javafx/font/coretext/CTGlyphLayout.class */
class CTGlyphLayout extends GlyphLayout {
    ...
    private long createCTLine(long fontRef, char[] chars, boolean rtl, int start, int length) {
        ...
        if (textRef != 0) {
            ...
            if (attributes != 0) {
                ...
                if (rtl) {
                    long paragraphStyleRef = OS.CTParagraphStyleCreate(1);
    ...
    private int getFontSlot(long runRef, CompositeFontResource fr, String name, int slot) {
        ...
        if (actualFont == 0 || (fontName = OS.CTFontCopyAttributeDisplayName(actualFont)) == null) {
            return -1;
        }
    ...
    public void layout(TextRun run, PGFont font, FontStrike strike, char[] text) {
        ...
        if (runs != 0) {
            ...
            for (int i2 = 0; i2 < runCount; i2++) {
                long runRef = OS.CFArrayGetValueAtIndex(runs, i2);
                if (runRef != 0) {

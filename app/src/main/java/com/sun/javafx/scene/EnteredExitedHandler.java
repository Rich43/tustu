...
/* loaded from: jfxrt.jar:com/sun/javafx/scene/EnteredExitedHandler.class */
public class EnteredExitedHandler extends BasicEventDispatcher {
    private final Object eventSource;
    private boolean eventTypeModified;

    public EnteredExitedHandler(Object eventSource) {
    ...
    public final Event dispatchCapturingEvent(Event event) {
        if (this.eventSource == event.getTarget()) {
            ...
            if (event.getEventType() == MouseDragEvent.MOUSE_DRAG_ENTERED_TARGET) {
                this.eventTypeModified = true;
                return ((MouseDragEvent) event).copyFor(this.eventSource, event.getTarget(), (EventType<? extends MouseEvent>) MouseDragEvent.MOUSE_DRAG_ENTERED);
            }
            if (event.getEventType() == MouseDragEvent.MOUSE_DRAG_EXITED_TARGET) {
                this.eventTypeModified = true;
                return ((MouseDragEvent) event).copyFor(this.eventSource, event.getTarget(), (EventType<? extends MouseEvent>) MouseDragEvent.MOUSE_DRAG_EXITED);
            }
    ...
    public final Event dispatchBubblingEvent(Event event) {
        if (this.eventTypeModified && this.eventSource == event.getTarget()) {
            ...
            if (event.getEventType() == MouseDragEvent.MOUSE_DRAG_ENTERED) {
                this.eventTypeModified = true;
                return ((MouseDragEvent) event).copyFor(this.eventSource, event.getTarget(), (EventType<? extends MouseEvent>) MouseDragEvent.MOUSE_DRAG_ENTERED_TARGET);
            }
            if (event.getEventType() == MouseDragEvent.MOUSE_DRAG_EXITED) {
                this.eventTypeModified = true;
                return ((MouseDragEvent) event).copyFor(this.eventSource, event.getTarget(), (EventType<? extends MouseEvent>) MouseDragEvent.MOUSE_DRAG_EXITED_TARGET);
            }

...
/* loaded from: rt.jar:com/sun/xml/internal/ws/api/pipe/ServerTubeAssemblerContext.class */
public class ServerTubeAssemblerContext {
    ...
    public Tube createHandlerTube(@NotNull Tube next) {
        if (!this.binding.getHandlerChain().isEmpty()) {
            HandlerTube cousin = new ServerLogicalHandlerTube(this.binding, this.seiModel, this.wsdlModel, next);
            next = cousin;
            if (this.binding instanceof SOAPBinding) {
                HandlerTube cousin2 = new ServerSOAPHandlerTube(this.binding, next, cousin);
                next = new ServerMessageHandlerTube(this.seiModel, this.binding, cousin2, cousin2);
            }
        }
        return next;
    }

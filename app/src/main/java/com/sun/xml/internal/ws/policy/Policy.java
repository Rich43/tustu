...
/* loaded from: rt.jar:com/sun/xml/internal/ws/policy/Policy.class */
public class Policy implements Iterable<AssertionSet> {
    ...
    private boolean add(AssertionSet set) {
        if (set == null || this.assertionSets.contains(set)) {
            return false;
        }
    ...
    private boolean addAll(Collection<AssertionSet> sets) {
        if (!$assertionsDisabled && (sets == null || sets.isEmpty())) {
            throw new AssertionError((Object) LocalizationMessages.WSP_0036_PRIVATE_METHOD_DOES_NOT_ACCEPT_NULL_OR_EMPTY_COLLECTION());
        }
        boolean result = true;
    ...
    public boolean equals(Object obj) {
        ...
        boolean result2 = result && this.assertionSets.size() == that.assertionSets.size() && this.assertionSets.containsAll(that.assertionSets);
        return result2;
    }

    public int hashCode() {
        int result = (37 * 17) + this.vocabulary.hashCode();
        return (37 * result) + this.assertionSets.hashCode();
    }
    ...
    StringBuffer toString(int indentLevel, StringBuffer buffer) {
        ...
        if (this.vocabulary.isEmpty()) {
            ...
            for (QName entry : this.vocabulary) {
                int i2 = index;
                index++;
                buffer.append(innerDoubleIndent).append(i2).append(". entry = '").append(entry.getNamespaceURI()).append(':').append(entry.getLocalPart()).append('\'').append(PolicyUtils.Text.NEW_LINE);
            }
        }

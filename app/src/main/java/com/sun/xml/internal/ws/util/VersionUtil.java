...
/* loaded from: rt.jar:com/sun/xml/internal/ws/util/VersionUtil.class */
public final class VersionUtil {
    ...
    public static int[] getCanonicalVersion(String version) {
        int[] canonicalVersion = {1, 1, 0, 0};
        StringTokenizer tokenizer = new StringTokenizer(version, ".");
        canonicalVersion[0] = Integer.parseInt(tokenizer.nextToken());
        String token = tokenizer.nextToken();
        if (token.indexOf("_") == -1) {
            canonicalVersion[1] = Integer.parseInt(token);
        } else {
            StringTokenizer subTokenizer = new StringTokenizer(token, "_");
        ...
        if (tokenizer.hasMoreTokens()) {
            ...
            if (token2.indexOf("_") == -1) {
                canonicalVersion[2] = Integer.parseInt(token2);
                if (tokenizer.hasMoreTokens()) {
                    canonicalVersion[3] = Integer.parseInt(tokenizer.nextToken());
                }
            } else {
                StringTokenizer subTokenizer2 = new StringTokenizer(token2, "_");
                canonicalVersion[2] = Integer.parseInt(subTokenizer2.nextToken());
                canonicalVersion[3] = Integer.parseInt(subTokenizer2.nextToken());
            }
        }

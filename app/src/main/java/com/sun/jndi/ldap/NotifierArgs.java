...
/* loaded from: rt.jar:com/sun/jndi/ldap/NotifierArgs.class */
final class NotifierArgs {
    ...
    NotifierArgs(String str, int i2, NamingListener namingListener) {
        this(str, "(objectclass=*)", null, namingListener);
        if (i2 != 1) {
            this.controls = new SearchControls();
            this.controls.setSearchScope(i2);
        }
    }

    NotifierArgs(String str, String str2, SearchControls searchControls, NamingListener namingListener) {
        this.sum = -1;
        this.name = str;
        this.filter = str2;

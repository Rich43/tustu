...
/* loaded from: rt.jar:com/sun/org/apache/xml/internal/dtm/ref/sax2dtm/SAX2RTFDTM.class */
public class SAX2RTFDTM extends SAX2DTM {
    ...
    public int getDocumentRoot(int nodeHandle) {
        int iMakeNodeIdentity = makeNodeIdentity(nodeHandle);
        while (true) {
            int id = iMakeNodeIdentity;
            if (id != -1) {
                if (_type(id) != 9) {
                    iMakeNodeIdentity = _parent(id);
                } else {
    ...
    protected int _documentRoot(int nodeIdentifier) {
        ...
        int i_parent = _parent(nodeIdentifier);
        while (true) {
            int parent = i_parent;
            if (parent != -1) {
                nodeIdentifier = parent;
                i_parent = _parent(nodeIdentifier);
            } else {

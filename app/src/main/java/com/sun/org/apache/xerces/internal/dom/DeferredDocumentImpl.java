...
/* loaded from: rt.jar:com/sun/org/apache/xerces/internal/dom/DeferredDocumentImpl.class */
public class DeferredDocumentImpl extends DocumentImpl implements DeferredNode {
    ...
    public int cloneNode(int nodeIndex, boolean deep) {
        ...
        if (deep) {
            int prevIndex = -1;
            int lastChild = getLastChild(nodeIndex, false);
            while (true) {
                int childIndex = lastChild;
                if (childIndex == -1) {
                    break;
                }
                ...
                lastChild = getRealPrevSibling(childIndex, false);
            }
        }
        return cloneIndex;
    }
    ...
    public int lookupElementDefinition(String elementName) {
        if (this.fNodeCount > 1) {
            ...
            while (true) {
                ...
                int nchunk = index >> 8;
                int nindex = index & 255;
                if (getChunkIndex(this.fNodeType, nchunk, nindex) != 10) {
                    chunkIndex = getChunkIndex(this.fNodePrevSib, nchunk, nindex);
                } else {
                    docTypeIndex = index;
                    break;
                }
            ...
            int chunkIndex2 = getChunkIndex(this.fNodeLastChild, docTypeIndex >> 8, docTypeIndex & 255);
            while (true) {
                int index2 = chunkIndex2;
                if (index2 != -1) {
                    int nchunk2 = index2 >> 8;
                    int nindex2 = index2 & 255;
                    if (getChunkIndex(this.fNodeType, nchunk2, nindex2) != 21 || getChunkValue(this.fNodeName, nchunk2, nindex2) != elementName) {
                        chunkIndex2 = getChunkIndex(this.fNodePrevSib, nchunk2, nindex2);
                    } else {
                        return index2;
                    }
                } else {
        ...
        switch (type) {
            case 1:
                if (this.fNamespacesEnabled) {
                    deferredElementDefinitionImpl = new DeferredElementNSImpl(this, nodeIndex);
                } else {
                    deferredElementDefinitionImpl = new DeferredElementImpl(this, nodeIndex);
                }
                if (this.fIdElement != null) {
                    ...
                    while (idIndex != -1) {
                        ...
                        if (name != null) {
                            putIdentifier0(name, deferredElementDefinitionImpl);
            ...
            case 2:
                if (this.fNamespacesEnabled) {
                    deferredElementDefinitionImpl = new DeferredAttrNSImpl(this, nodeIndex);
                    break;
                } else {
                    deferredElementDefinitionImpl = new DeferredAttrImpl(this, nodeIndex);
                    break;
                }
            case 3:
                deferredElementDefinitionImpl = new DeferredTextImpl(this, nodeIndex);
                break;
            case 4:
                deferredElementDefinitionImpl = new DeferredCDATASectionImpl(this, nodeIndex);
                break;
            case 5:
                deferredElementDefinitionImpl = new DeferredEntityReferenceImpl(this, nodeIndex);
                break;
            case 6:
                deferredElementDefinitionImpl = new DeferredEntityImpl(this, nodeIndex);
                break;
            case 7:
                deferredElementDefinitionImpl = new DeferredProcessingInstructionImpl(this, nodeIndex);
                break;
            case 8:
                deferredElementDefinitionImpl = new DeferredCommentImpl(this, nodeIndex);
                break;
            case 9:
                deferredElementDefinitionImpl = this;
                break;
            case 10:
                deferredElementDefinitionImpl = new DeferredDocumentTypeImpl(this, nodeIndex);
            ...
            case 12:
                deferredElementDefinitionImpl = new DeferredNotationImpl(this, nodeIndex);
                break;
            case 21:
                deferredElementDefinitionImpl = new DeferredElementDefinitionImpl(this, nodeIndex);
                break;
        }
    ...
    public String getNodeValueString(int nodeIndex, boolean free) {
        ...
        if (type == 3) {
            ...
            if (prevSib != -1 && getNodeType(prevSib, false) == 3) {
                ...
                do {
                    int chunk2 = prevSib >> 8;
                    int index2 = prevSib & 255;
                ...
                for (int i2 = chunkCount - 1; i2 >= 0; i2--) {
    ...
    public Object getTypeInfo(int nodeIndex) {
        ...
        if (value != null) {
            ...
            RefCount c2 = (RefCount) this.fNodeValue[chunk][256];
            c2.fCount--;
            if (c2.fCount == 0) {
    ...
    public String getAttribute(int elemIndex, String name) {
        ...
        int echunk = elemIndex >> 8;
        int eindex = elemIndex & 255;
        int chunkIndex = getChunkIndex(this.fNodeExtra, echunk, eindex);
        while (true) {
            int attrIndex = chunkIndex;
            if (attrIndex != -1) {
                int achunk = attrIndex >> 8;
                int aindex = attrIndex & 255;
                ...
                chunkIndex = getChunkIndex(this.fNodePrevSib, achunk, aindex);
            } else {
                return null;
            }
        }
    ...
    protected void synchronizeData() {
        ...
        if (this.fIdElement != null) {
            ...
            int i2 = 0;
            while (i2 < this.fIdCount) {
                int elementNodeIndex = this.fIdElement[i2];
                String idName = this.fIdName[i2];
                if (idName != null) {
                    ...
                    do {
                        ...
                        int pchunk = index >> 8;
                        int pindex = index & 255;
                        index = getChunkIndex(this.fNodeParent, pchunk, pindex);
                    } while (index != -1);
                    Node place = this;
                    for (int j2 = path.size() - 2; j2 >= 0; j2--) {
                        int index2 = path.elementAt(j2);
                        Node lastChild = place.getLastChild();
                        while (true) {
                    ...
                    this.fIdName[i2] = null;
                    while (i2 + 1 < this.fIdCount && this.fIdElement[i2 + 1] == elementNodeIndex) {
                        i2++;
                        String idName2 = this.fIdName[i2];
                        if (idName2 != null) {
    ...
    protected void synchronizeChildren() {
        ...
        while (true) {
            ...
            lastChild = getPrevSibling(index);
        }
        if (first != null) {
    ...
    protected final void synchronizeChildren(AttrImpl a2, int nodeIndex) {
        boolean orig = getMutationEvents();
        setMutationEvents(false);
        a2.needsSyncChildren(false);
        int last = getLastChild(nodeIndex);
        int prev = getPrevSibling(last);
        if (prev == -1) {
            a2.value = getNodeValueString(nodeIndex);
            a2.hasStringValue(true);
        } else {
            ChildNode firstNode = null;
            ChildNode lastNode = null;
            int prevSibling = last;
            while (true) {
                int index = prevSibling;
                if (index == -1) {
                    break;
                }
                ChildNode node = (ChildNode) getNodeObject(index);
                ...
                firstNode = node;
                prevSibling = getPrevSibling(index);
            }
            if (lastNode != null) {
                a2.value = firstNode;
                firstNode.isFirstChild(true);
                a2.lastChild(lastNode);
            }
            a2.hasStringValue(false);
        }
        setMutationEvents(orig);
    }

    protected final void synchronizeChildren(ParentNode p2, int nodeIndex) {
        boolean orig = getMutationEvents();
        setMutationEvents(false);
        p2.needsSyncChildren(false);
        ChildNode firstNode = null;
        ChildNode lastNode = null;
        int lastChild = getLastChild(nodeIndex);
        while (true) {
            int index = lastChild;
            if (index == -1) {
                break;
        ...
        if (lastNode != null) {
            p2.firstChild = firstNode;
            firstNode.isFirstChild(true);
            p2.lastChild(lastNode);
        }
        setMutationEvents(orig);
    }
    ...
    protected void ensureCapacity(int chunk) {
        if (this.fNodeType == null) {
            ...
            int newsize = chunk * 2;
            ?? r0 = new int[newsize];
            System.arraycopy(this.fNodeType, 0, r0, 0, chunk);
            this.fNodeType = r0;
            ?? r02 = new Object[newsize];
            System.arraycopy(this.fNodeName, 0, r02, 0, chunk);
            this.fNodeName = r02;
            ?? r03 = new Object[newsize];
            System.arraycopy(this.fNodeValue, 0, r03, 0, chunk);
            this.fNodeValue = r03;
            ?? r04 = new int[newsize];
            System.arraycopy(this.fNodeParent, 0, r04, 0, chunk);
            this.fNodeParent = r04;
            ?? r05 = new int[newsize];
            System.arraycopy(this.fNodeLastChild, 0, r05, 0, chunk);
            this.fNodeLastChild = r05;
            ?? r06 = new int[newsize];
            System.arraycopy(this.fNodePrevSib, 0, r06, 0, chunk);
            this.fNodePrevSib = r06;
            ?? r07 = new Object[newsize];
            System.arraycopy(this.fNodeURI, 0, r07, 0, chunk);
            this.fNodeURI = r07;
            ?? r08 = new int[newsize];
            System.arraycopy(this.fNodeExtra, 0, r08, 0, chunk);
            this.fNodeExtra = r08;
    ...
    /* loaded from: rt.jar:com/sun/org/apache/xerces/internal/dom/DeferredDocumentImpl$RefCount.class */
    ...
    private final String setChunkValue(Object[][] data, Object value, int chunk, int index) {
        ...
        if (ovalue == null) {
            RefCount c2 = (RefCount) dataChunk[256];
            c2.fCount++;
        }
        dataChunk[index] = value;
        return ovalue;
    ...
    private final String clearChunkValue(Object[][] data, int chunk, int index) {
        ...
        if (value != null) {
            ...
            RefCount c2 = (RefCount) data[chunk][256];
            c2.fCount--;
            if (c2.fCount == 0) {
                data[chunk] = null;
            }
    ...
    /* loaded from: rt.jar:com/sun/org/apache/xerces/internal/dom/DeferredDocumentImpl$IntVector.class */

...
/* loaded from: nashorn.jar:jdk/internal/dynalink/support/Guards.class */
public class Guards {
    ...
    static {
        ...
        IS_INSTANCE = Lookup.PUBLIC.findVirtual(Class.class, "isInstance", MethodType.methodType(Boolean.TYPE, (Class<?>) Object.class));
    ...
    public static MethodHandle isInstance(Class<?> clazz, int pos, MethodType type) {
        ...
        if (clazz.isAssignableFrom(declaredType)) {
            LOG.log(Level.WARNING, "isInstanceGuardAlwaysTrue", new Object[]{clazz.getName(), Integer.valueOf(pos), type, DynamicLinker.getLinkedCallSiteLocation()});
            return constantTrue(type);
        }
        if (!declaredType.isAssignableFrom(clazz)) {
            LOG.log(Level.WARNING, "isInstanceGuardAlwaysFalse", new Object[]{clazz.getName(), Integer.valueOf(pos), type, DynamicLinker.getLinkedCallSiteLocation()});
            return constantFalse(type);
        }
    ...
    public static MethodHandle isArray(int pos, MethodType type) {
        ...
        if (declaredType.isArray()) {
            LOG.log(Level.WARNING, "isArrayGuardAlwaysTrue", new Object[]{Integer.valueOf(pos), type, DynamicLinker.getLinkedCallSiteLocation()});
            return constantTrue(type);
        }
        if (!declaredType.isAssignableFrom(Object[].class)) {
            LOG.log(Level.WARNING, "isArrayGuardAlwaysFalse", new Object[]{Integer.valueOf(pos), type, DynamicLinker.getLinkedCallSiteLocation()});
            return constantFalse(type);
        }
    ...
    private static MethodHandle asType(MethodHandle test, int pos, MethodType type) {
        ...
        if (!$assertionsDisabled && type.parameterCount() <= 0) {
            throw new AssertionError();
        }
        if (!$assertionsDisabled && (pos < 0 || pos >= type.parameterCount())) {
            throw new AssertionError();
        }
        if (!$assertionsDisabled && test.type().parameterCount() != 1) {
            throw new AssertionError();
        }
        if ($assertionsDisabled || test.type().returnType() == Boolean.TYPE) {
    ...
    private static boolean isArray(Object o2) {
        return o2 != null && o2.getClass().isArray();
    }

    private static boolean isOfClass(Class<?> c2, Object o2) {
        return o2 != null && o2.getClass() == c2;
    }

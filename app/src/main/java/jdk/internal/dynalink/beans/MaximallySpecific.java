...
/* loaded from: nashorn.jar:jdk/internal/dynalink/beans/MaximallySpecific.class */
class MaximallySpecific {
    ...
    static {
        ...
        METHOD_HANDLE_TYPE_GETTER = new MethodTypeGetter<MethodHandle>() { // from class: jdk.internal.dynalink.beans.MaximallySpecific.1
            ...
            public MethodType getMethodType(MethodHandle t2) {
                return t2.type();
            }
        };
        DYNAMIC_METHOD_TYPE_GETTER = new MethodTypeGetter<SingleDynamicMethod>() { // from class: jdk.internal.dynalink.beans.MaximallySpecific.2
            /* JADX INFO: Access modifiers changed from: package-private */
            @Override // jdk.internal.dynalink.beans.MaximallySpecific.MethodTypeGetter
            public MethodType getMethodType(SingleDynamicMethod t2) {
                return t2.getMethodType();
            }
        };
    }
    ...
    /* loaded from: nashorn.jar:jdk/internal/dynalink/beans/MaximallySpecific$MethodTypeGetter.class */
    private static abstract class MethodTypeGetter<T> {
        abstract MethodType getMethodType(T t2);
    ...
    private static <T> List<T> getMaximallySpecificMethods(List<T> methods, boolean varArgs, Class<?>[] argTypes, LinkerServices ls, MethodTypeGetter<T> methodTypeGetter) {
        ...
        for (T m2 : methods) {
            methodTypeGetter.getMethodType(m2);
    ...
    private static ConversionComparator.Comparison isMoreSpecific(MethodType t1, MethodType t2, boolean varArgs, Class<?>[] argTypes, LinkerServices ls) {
        ...
        for (int i2 = 1; i2 < maxPc; i2++) {
            Class<?> c1 = getParameterClass(t1, pc1, i2, varArgs);
            Class<?> c2 = getParameterClass(t2, pc2, i2, varArgs);
            if (c1 != c2) {
                ConversionComparator.Comparison cmp = compare(c1, c2, argTypes, i2, ls);
    ...
    private static ConversionComparator.Comparison compare(Class<?> c1, Class<?> c2, Class<?>[] argTypes, int i2, LinkerServices cmp) {
        ConversionComparator.Comparison c3;
        if (cmp != null && (c3 = cmp.compareConversion(argTypes[i2], c1, c2)) != ConversionComparator.Comparison.INDETERMINATE) {
            return c3;
        }
    ...
    private static Class<?> getParameterClass(MethodType t2, int l2, int i2, boolean varArgs) {
        return (!varArgs || i2 < l2 - 1) ? t2.parameterType(i2) : t2.parameterType(l2 - 1).getComponentType();
    }
}

...
/* loaded from: nashorn.jar:jdk/internal/dynalink/beans/SingleDynamicMethod.class */
abstract class SingleDynamicMethod extends DynamicMethod {
    ...
    static String getMethodNameWithSignature(MethodType type, String methodName, boolean withReturnType) {
        ...
        if (withReturnType) {
            b2.append((CharSequence) typeStr, retTypeIndex, typeStr.length()).append(' ');
        }
        return b2.append(methodName).append('(').append((CharSequence) typeStr, secondParamIndex, retTypeIndex).toString();
    }

    static MethodHandle getInvocation(MethodHandle target, MethodType callSiteType, LinkerServices linkerServices) throws RuntimeException {
        ...
        if (argsLen == paramsLen) {
            ...
            if ($assertionsDisabled || isCustomConvertible) {
    ...
    private static boolean typeMatchesDescription(String paramTypes, MethodType type) {
        ...
        for (int i2 = 1; i2 < type.parameterCount(); i2++) {
            if (!tok.hasMoreTokens() || !typeNameMatches(tok.nextToken(), type.parameterType(i2))) {
                return false;
            }
        }

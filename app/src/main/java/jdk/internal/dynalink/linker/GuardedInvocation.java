...
/* loaded from: nashorn.jar:jdk/internal/dynalink/linker/GuardedInvocation.class */
public class GuardedInvocation {
    ...
    public MethodHandle compose(MethodHandle guardFallback, MethodHandle switchpointFallback, MethodHandle catchFallback) {
        ...
        MethodHandle catchGuarded = this.exception == null ? guarded : Lookup.MH.catchException(guarded, this.exception, MethodHandles.dropArguments(catchFallback, 0, (Class<?>[]) new Class[]{this.exception}));
        if (this.switchPoints == null) {
            return catchGuarded;
        }
    ...
    private static void assertType(MethodHandle mh, MethodType type) {
        if (!mh.type().equals((Object) type)) {
            throw new WrongMethodTypeException("Expected type: " + ((Object) type) + " actual type: " + ((Object) mh.type()));
        }
    }
}

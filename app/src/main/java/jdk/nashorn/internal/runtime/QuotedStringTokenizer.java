...
/* loaded from: nashorn.jar:jdk/nashorn/internal/runtime/QuotedStringTokenizer.class */
public final class QuotedStringTokenizer {
    ...
    */
    ...
    private String stripQuotes(String value0) {
        ...
        for (char q2 : this.quotes) {
            if (value.length() >= 2 && value.startsWith("" + q2) && value.endsWith("" + q2)) {
                value = value.substring(1, value.length() - 1).replace(FXMLLoader.ESCAPE_PREFIX + q2, "" + q2);
            }
        }
        return value;
    }
            ...
            for (char q2 : this.quotes) {
                if (c2 == q2) {
                    if (quoteStack.isEmpty()) {
                        quoteStack.push(Character.valueOf(c2));
                    } else {

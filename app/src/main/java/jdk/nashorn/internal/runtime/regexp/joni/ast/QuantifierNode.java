...
/* loaded from: nashorn.jar:jdk/nashorn/internal/runtime/regexp/joni/ast/QuantifierNode.class */
public final class QuantifierNode extends StateNode {
    ...
    private static final String[] ReduceQStr = {"", "", "*", "*?", "??", "+ and ??", "+? and ?"};
    public static final int REPEAT_INFINITE = -1;

    /* loaded from: nashorn.jar:jdk/nashorn/internal/runtime/regexp/joni/ast/QuantifierNode$ReduceType.class */
    enum ReduceType {
        ASIS,
        DEL,
        A,
    ...
    protected int popularNum() {
        if (this.greedy) {
            ...
            if (this.lower == 1 && isRepeatInfinite(this.upper)) {
                return 2;
            }
            return -1;
        }
    ...
    public int setQuantifier(Node tgt, boolean group, ScanEnvironment env, char[] chars, int p2, int end) {
        StringNode n2;
        ...
        switch (tgt.getType()) {
            case 0:
                if (!group) {
                    ...
                    if (sn.canBeSplit() && (n2 = sn.splitLastChar()) != null) {
                        setTarget(n2);
                        return 2;
                    }
                }
    ...
    public static boolean isRepeatInfinite(int n2) {
        return n2 == -1;
    }
}

...
/* loaded from: nashorn.jar:jdk/nashorn/internal/runtime/regexp/joni/Parser.class */
class Parser extends Lexer {
    ...
    protected Parser(ScanEnvironment env, char[] chars, int p2, int end) {
        ...
        while (this.token.type != TokenType.CC_CLOSE) {
            ...
            switch (this.token.type) {
                ...
                case RAW_BYTE:
                    ...
                    break;
                case CODE_POINT:
                    arg.f12883v = this.token.getCode();
                    arg.vIsRaw = true;
                ...
                case CC_RANGE:
                    if (arg.state == CCSTATE.VALUE) {
                        ...
                        if (this.token.type == TokenType.CC_CLOSE) {
                            ...
                            this.env.ccEscWarn(LanguageTag.SEP);
                            parseCharClassRangeEndVal(cc, arg);
                            break;
                        } else {
    ...
    private void parseCharClassValEntry(CClassNode cc, CClassNode.CCStateArg arg) {
        arg.inType = arg.f12883v <= 255 ? CCVALTYPE.SB : CCVALTYPE.CODE_POINT;
        parseCharClassValEntry2(cc, arg);
    }
    ...
    private Node parseExpTkByte(boolean group) {
        StringNode node = new StringNode(this.chars, this.token.backP, this.f12880p);
        while (true) {
            fetchToken();
            if (this.token.type == TokenType.STRING) {
                if (this.token.backP == node.end) {
                    node.end = this.f12880p;
                } else {
                    node.cat(this.chars, this.token.backP, this.f12880p);
                }
            } else {
                return parseExpRepeat(node, group);
            }
    ...
    private Node parseSubExp(TokenType term) {
        ...
        if (this.token.type == TokenType.ALT) {
            ...
            while (true) {
                ...
                fetchToken();
                t2.setCdr(ConsAltNode.newAltNode(parseBranch(term), null));
                consAltNode = t2.cdr;
            }
            if (this.token.type != term) {

...
/* loaded from: nashorn.jar:jdk/nashorn/internal/runtime/regexp/RegExpScanner.class */
final class RegExpScanner extends Scanner {
    ...
    /* loaded from: nashorn.jar:jdk/nashorn/internal/runtime/regexp/RegExpScanner$Capture.class */
    ...
    private void processForwardReferences() {
        ...
        while (iterator.hasNext()) {
            ...
            if (num > this.caps.size()) {
                ...
                this.sb.insert(pos, (CharSequence) buffer);
            }
        }
    ...
    public static RegExpScanner scan(String string) {
        ...
        try {
            ...
            if (scanner.position != string.length()) {
                String p2 = scanner.getStringBuilder().toString();
                throw new PatternSyntaxException(string, p2, p2.length() + 1);
            }
            scanner.processForwardReferences();
            return scanner;
        } catch (Exception e2) {
            throw new PatternSyntaxException(e2.getMessage(), string, scanner.position);
        }
    }
    ...
    BitVector getGroupsInNegativeLookahead() {
        BitVector vec = null;
        for (int i2 = 0; i2 < this.caps.size(); i2++) {
            Capture cap = this.caps.get(i2);
    ...
    private boolean patternCharacter() {
        ...
        switch (this.ch0) {
            ...
            case ']':
            case '}':
                int n2 = this.expected.get(Character.valueOf(this.ch0)).intValue();
                if (n2 != 0) {
                    return false;
                }
                break;
    ...
    private boolean decimalEscape() {
        ...
        if (this.ch0 == '0' && !isOctalDigit(this.ch1)) {
            skip(1);
            this.sb.append(Localizable.NOT_LOCALIZABLE);
            return true;
        }
        if (isDecimalDigit(this.ch0)) {
            ...
            if (decimalValue <= this.caps.size()) {

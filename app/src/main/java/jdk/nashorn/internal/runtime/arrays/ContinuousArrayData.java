...
@Logger(name = "arrays")
/* loaded from: nashorn.jar:jdk/nashorn/internal/runtime/arrays/ContinuousArrayData.class */
public abstract class ContinuousArrayData extends ArrayData {
    ...
    public abstract MethodHandle getElementGetter(Class<?> cls, int i2);
    ...
    protected MethodHandle getContinuousElementGetter(Class<? extends ContinuousArrayData> clazz, MethodHandle getHas, Class<?> returnType, int programPoint) {
        ...
        if (isOptimistic && ti < fti) {
            mh = Lookup.MH.insertArguments(ArrayData.THROW_UNWARRANTED.methodHandle(), 1, Integer.valueOf(programPoint));
        }
        MethodHandle mh2 = Lookup.MH.asType(mh, mh.type().changeReturnType(returnType).changeParameterType(0, clazz));
        if (!isOptimistic) {
    ...
    public GuardedInvocation findFastGetIndexMethod(Class<? extends ArrayData> clazz, CallSiteDescriptor desc, LinkRequest request) {
        ...
        if (ContinuousArrayData.class.isAssignableFrom(clazz) && indexType == Integer.TYPE) {
            ...
            if (has(index)) {
                ...
                if (getElement != null) {
                    MethodHandle getElement2 = Lookup.MH.filterArguments(getElement, 0, Lookup.MH.asType(getArray, getArray.type().changeReturnType(clazz)));
                    MethodHandle guard = Lookup.MH.insertArguments(FAST_ACCESS_GUARD, 0, clazz);
                    return new GuardedInvocation(getElement2, guard, (SwitchPoint) null, (Class<? extends Throwable>) ClassCastException.class);
                }
                return null;
            }
            return null;
    ...
    public GuardedInvocation findFastSetIndexMethod(Class<? extends ArrayData> clazz, CallSiteDescriptor desc, LinkRequest request) {
        ...
        if (ContinuousArrayData.class.isAssignableFrom(clazz) && indexType == Integer.TYPE) {
            ...
            if (hasRoomFor(index) && (setElement = getElementSetter(elementType)) != null) {
                MethodHandle getArray = ScriptObject.GET_ARRAY.methodHandle();
                MethodHandle setElement2 = Lookup.MH.filterArguments(setElement, 0, Lookup.MH.asType(getArray, getArray.type().changeReturnType(getClass())));
                MethodHandle guard = Lookup.MH.insertArguments(FAST_ACCESS_GUARD, 0, clazz);
                return new GuardedInvocation(setElement2, guard, (SwitchPoint) null, (Class<? extends Throwable>) ClassCastException.class);
            }
            return null;
        }
        return null;

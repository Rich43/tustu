...
/* loaded from: nashorn.jar:jdk/nashorn/internal/runtime/linker/ClassAndLoader.class */
final class ClassAndLoader {
    ...
    static ClassAndLoader getDefiningClassAndLoaderPrivileged(Class<?>[] types) {
        ...
        b2.append(it.next().getRepresentativeClass().getCanonicalName());
        while (it.hasNext()) {
            b2.append(", ").append(it.next().getRepresentativeClass().getCanonicalName());
        }
        throw ECMAErrors.typeError("extend.ambiguous.defining.class", b2.toString());
    }
    ...
    private static Collection<ClassAndLoader> getClassLoadersForTypes(Class<?>[] types) {
        ...
        for (Class<?> c2 : types) {
            ClassAndLoader cl = new ClassAndLoader(c2, true);

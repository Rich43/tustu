...
/* loaded from: nashorn.jar:jdk/nashorn/internal/runtime/regexp/joni/ast/CClassNode.class */
public final class CClassNode extends Node {
    ...
    public final BitSet f12882bs = new BitSet();
    ...
    /* loaded from: nashorn.jar:jdk/nashorn/internal/runtime/regexp/joni/ast/CClassNode$CCStateArg.class */
    ...
    public boolean isEmpty() {
        return this.mbuf == null && this.f12882bs.isEmpty();
    }
    ...
    public void and(CClassNode other) {
        ...
        bsr1.and(bsr2);
        if (bsr1 != this.f12882bs) {
            this.f12882bs.copy(bsr1);
            BitSet bitSet = this.f12882bs;
        }
        if (not1) {
            this.f12882bs.invert();
        }
    ...
    public void addCTypeByRange(int ct, boolean not, int sbOut, int[] mbr) {
        ...
        if (!not) {
            int i2 = 0;
            while (i2 < n2) {
                for (int j2 = mbr[(i2 * 2) + 1]; j2 <= mbr[(i2 * 2) + 2]; j2++) {
                    if (j2 >= sbOut) {
                        if (j2 >= mbr[(i2 * 2) + 1]) {
                            addCodeRangeToBuf(j2, mbr[(i2 * 2) + 2]);
                            i2++;
                        }
                        while (i2 < n2) {
                    ...
                    this.f12882bs.set(j2);
                }
                i2++;
            }
            for (int i3 = 0; i3 < n2; i3++) {
                addCodeRangeToBuf(mbr[(2 * i3) + 1], mbr[(2 * i3) + 2]);
            }
            return;
        }
        int prev = 0;
        for (int i4 = 0; i4 < n2; i4++) {
            for (int j3 = prev; j3 < mbr[(2 * i4) + 1]; j3++) {
                if (j3 >= sbOut) {
                    int prev2 = sbOut;
                    for (int i5 = 0; i5 < n2; i5++) {
                        if (prev2 < mbr[(2 * i5) + 1]) {
                            addCodeRangeToBuf(prev2, mbr[(i5 * 2) + 1] - 1);
                        }
                        prev2 = mbr[(i5 * 2) + 2] + 1;
                    }
                ...
                this.f12882bs.set(j3);
            }
            prev = mbr[(2 * i4) + 2] + 1;
        }
        for (int j4 = prev; j4 < sbOut; j4++) {
            this.f12882bs.set(j4);
        }
        int prev3 = sbOut;
        for (int i6 = 0; i6 < n2; i6++) {
            if (prev3 < mbr[(2 * i6) + 1]) {
                addCodeRangeToBuf(prev3, mbr[(i6 * 2) + 1] - 1);
            }
            prev3 = mbr[(i6 * 2) + 2] + 1;
        }
            ...
            case CharacterType.f12889W /* 268 */:
                ...
                if (env.syntax != Syntax.JAVASCRIPT || ct != 9) {
                    ...
                    for (int c3 = 0; c3 < 256; c3++) {
                        if ((AsciiCtypeTable[c3] & (1 << ct)) != 0) {
                            this.f12882bs.set(c3);
                        }
                    }
                    return;
                }
        ...
        switch (ct) {
                ...
                if (not) {
                    for (int c6 = 0; c6 < 256; c6++) {
                        if (!EncodingHelper.isCodeCType(c6, ct)) {
                            this.f12882bs.set(c6);
                        }
                    }
                    return;
                }
                for (int c7 = 0; c7 < 256; c7++) {
                    if (EncodingHelper.isCodeCType(c7, ct)) {
                        this.f12882bs.set(c7);
                    }
                }
                addAllMultiByteRange();
                return;
            case 12:
                ...
                for (int c9 = 0; c9 < 256; c9++) {
                    if (!EncodingHelper.isWord(c9)) {
                        this.f12882bs.set(c9);
                    }
                }
                return;
            default:
            ...
            case RANGE:
                if (arg.inType == arg.type) {
                    if (arg.inType == CCVALTYPE.SB) {
                        if (arg.vs > 255 || arg.f12883v > 255) {
                            throw new ValueException(ErrorMessages.ERR_INVALID_CODE_POINT_VALUE);
                        }
                        if (arg.vs > arg.f12883v) {
                            ...
                            this.f12882bs.setRange(arg.vs, arg.f12883v);
                        }
                    } else {
                        addCodeRange(env, arg.vs, arg.f12883v);
                    }
                    arg.state = CCSTATE.COMPLETE;
                    break;
                } else if (arg.vs > arg.f12883v) {
                    if (env.syntax.allowEmptyRangeInCC()) {
                        arg.state = CCSTATE.COMPLETE;
                        break;
                    } else {
                    ...
                    this.f12882bs.setRange(arg.vs, arg.f12883v < 255 ? arg.f12883v : 255);
                    addCodeRange(env, arg.vs, arg.f12883v);
                    arg.state = CCSTATE.COMPLETE;
                }
                break;

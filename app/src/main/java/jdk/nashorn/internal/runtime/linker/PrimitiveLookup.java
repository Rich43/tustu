...
/* loaded from: nashorn.jar:jdk/nashorn/internal/runtime/linker/PrimitiveLookup.class */
public final class PrimitiveLookup {
    ...
    public static GuardedInvocation lookupPrimitive(LinkRequest request, MethodHandle guard, ScriptObject wrappedReceiver, MethodHandle wrapFilter, MethodHandle protoFilter) {
        ...
        String name;
        FindProperty find;
        desc = request.getCallSiteDescriptor();
    ...
    private static GuardedInvocation getPrimitiveSetter(String name, MethodHandle guard, MethodHandle wrapFilter, boolean isStrict) {
        MethodHandle filter;
        MethodHandle target;
        MethodHandle filter2 = Lookup.MH.asType(wrapFilter, wrapFilter.type().changeReturnType(ScriptObject.class));
        if (name == null) {
            filter = Lookup.MH.dropArguments(filter2, 1, Object.class, Object.class);
            target = Lookup.MH.insertArguments(PRIMITIVE_SETTER, 3, Boolean.valueOf(isStrict));
        } else {
            filter = Lookup.MH.dropArguments(filter2, 1, Object.class);
            target = Lookup.MH.insertArguments(PRIMITIVE_SETTER, 2, name, Boolean.valueOf(isStrict));
        }

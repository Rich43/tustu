...
/* loaded from: nashorn.jar:jdk/nashorn/internal/runtime/regexp/joni/Lexer.class */
class Lexer extends ScannerSupport {
    protected final ScanEnvironment env;
    protected final Syntax syntax;
    protected final Token token;

    protected Lexer(ScanEnvironment env, char[] chars, int p2, int end) {
        super(chars, p2, end);
    ...
    private int fetchRangeQuantifier() {
        ...
        if (!synAllow) {
            this.f12881c = peek();
            if (this.f12881c == 41 || this.f12881c == 40 || this.f12881c == 124) {
                throw new SyntaxException(ErrorMessages.ERR_END_PATTERN_AT_LEFT_BRACE);
            }
        }
        ...
        if (this.syntax.opEscBraceInterval()) {
            if (this.f12881c != this.syntax.metaCharTable.esc) {
                return invalidRangeQuantifier(synAllow);
            }
            fetch();
        }
        ...
        switch (this.f12881c) {
            ...
            case 77:
                if (this.syntax.op2EscCapitalMBarMeta()) {
                    ...
                    fetch();
                    if (this.f12881c == this.syntax.metaCharTable.esc) {
                        this.f12881c = fetchEscapedValue();
                    }
                    this.f12881c = (this.f12881c & 255) | 128;
                } else {
                    fetchEscapedValueBackSlash();
                }
                return this.f12881c;
    ...
    private void fetchEscapedValueBackSlash() {
        this.f12881c = this.env.convertBackslashValue(this.f12881c);
    }
    ...
    private void fetchTokenInCCFor_x() {
        ...
        if (this.f12880p > last + 1 && left() && peekIs(125)) {
            inc();
    ...
    protected final TokenType fetchTokenInCC() {
        ...
        this.token.setC(this.f12881c);
        this.token.escaped = false;
        if (this.f12881c == 93) {
            this.token.type = TokenType.CC_CLOSE;
        } else if (this.f12881c == 45) {
            this.token.type = TokenType.CC_RANGE;
        } else if (this.f12881c == this.syntax.metaCharTable.esc) {
            ...
            switch (this.f12881c) {
                ...
                case 83:
                    ...
                    break;
                case 87:
                    fetchTokenInCCFor_charType(true, CharacterType.f12889W);
                    break;
                case 100:
                ...
                case 117:
                    ...
                    break;
                case 119:
                    fetchTokenInCCFor_charType(false, CharacterType.f12889W);
                    break;
                case 120:
                    fetchTokenInCCFor_x();
                    break;
            }
        } else if (this.f12881c == 38) {
    ...
    private void fetchTokenFor_digit() {
        ...
        if (this.f12881c == 56 || this.f12881c == 57) {
            this.f12880p = last;
            inc();
        } else {
            this.f12880p = last;
    ...
    private void fetchTokenFor_zero() {
        ...
        int last = this.f12880p;
        int num = scanUnsignedOctalNumber(this.f12881c == 48 ? 2 : 3);
        if (num < 0) {
            throw new ValueException(ErrorMessages.ERR_TOO_BIG_NUMBER);
        }
        if (this.f12880p == last) {
    ...
    private void fetchTokenFor_metaChars() {
        if (this.f12881c == this.syntax.metaCharTable.anyChar) {
            this.token.type = TokenType.ANYCHAR;
            return;
        }
        if (this.f12881c == this.syntax.metaCharTable.anyTime) {
            fetchTokenFor_repeat(0, -1);
            return;
        }
        if (this.f12881c == this.syntax.metaCharTable.zeroOrOneTime) {
            fetchTokenFor_repeat(0, 1);
            return;
        }
        if (this.f12881c == this.syntax.metaCharTable.oneOrMoreTime) {
            fetchTokenFor_repeat(1, -1);
        } else if (this.f12881c == this.syntax.metaCharTable.anyCharAnyTime) {
    ...
    protected final TokenType fetchToken() {
        while (left()) {
            ...
            this.token.backP = this.f12880p;
            fetch();
            if (this.f12881c == this.syntax.metaCharTable.esc && !this.syntax.op2IneffectiveEscape()) {
                if (!left()) {
                    throw new SyntaxException(ErrorMessages.ERR_END_PATTERN_AT_ESCAPE);
                }
                this.token.backP = this.f12880p;
                ...
                switch (this.f12881c) {
                    ...
                    default:
                        ...
                        if (this.token.getC() != num) {
                            ...
                            this.f12880p = this.token.backP + 1;
                            break;
                        }
                    case 48:
                        fetchTokenFor_zero();
                ...
                this.token.setC(this.f12881c);
                this.token.escaped = false;
                if (this.f12881c != 0 && this.syntax.opVariableMetaCharacters()) {
                    ...
                    switch (this.f12881c) {
                        ...
                        case 35:
                            if (!Option.isExtend(this.env.option)) {
                                ...
                                while (left()) {
                                    fetch();
                                    if (EncodingHelper.isNewLine(this.f12881c)) {
                                        break;
                                    }
                                }
                            }

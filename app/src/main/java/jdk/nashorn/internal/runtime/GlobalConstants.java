...
@Logger(name = "const")
/* loaded from: nashorn.jar:jdk/nashorn/internal/runtime/GlobalConstants.class */
public final class GlobalConstants implements Loggable {
    ...
    /* loaded from: nashorn.jar:jdk/nashorn/internal/runtime/GlobalConstants$Access.class */
    ...
    private synchronized Object invalidateSwitchPoint(Object obj, Access acc) {
        if (this.log.isEnabled()) {
            this.log.info("*** Invalidating switchpoint " + ((Object) acc.getSwitchPoint()) + " for receiver=" + obj + " access=" + ((Object) acc));
        }
        acc.invalidateOnce();
        if (acc.mayRetry()) {
            if (this.log.isEnabled()) {
                this.log.info("Retry is allowed for " + ((Object) acc) + "... Creating a new switchpoint.");
            }
            acc.newSwitchPoint();
        ...
        synchronized (this) {
            ...
            if (acc.mayRetry() && !this.invalidatedForever.get()) {
                if (acc.hasBeenInvalidated()) {
                    this.log.info("New chance for " + ((Object) acc));
                    acc.newSwitchPoint();
                }
                ...
                this.log.info("Linked setter " + DebugLogger.quote(name) + " " + ((Object) acc.getSwitchPoint()));
                return new GuardedInvocation(mh, inv.getGuard(), acc.getSwitchPoint(), inv.getException());
            }
            if (this.log.isEnabled()) {
                this.log.fine("*** SET: Giving up on " + DebugLogger.quote(name) + " - retry count has exceeded " + ((Object) DynamicLinker.getLinkedCallSiteLocation()));
            }
            return null;
        }
    }

    public static MethodHandle staticConstantGetter(Object c2) {
        return Lookup.MH.dropArguments(JSType.unboxConstant(c2), 0, Object.class);
    }

    private MethodHandle constantGetter(Object c2) {
        MethodHandle mh = staticConstantGetter(c2);
        if (this.log.isEnabled()) {
    ...
    GuardedInvocation findGetMethod(FindProperty find, ScriptObject receiver, CallSiteDescriptor desc) {
        ...
        synchronized (this) {
            ...
            Object c2 = find.getObjectValue();
            if (this.log.isEnabled()) {
                this.log.fine("Trying to link constant GETTER " + ((Object) acc) + " value = " + c2);
            }
            if (acc.hasBeenInvalidated() || acc.guardFailed() || this.invalidatedForever.get()) {
                if (this.log.isEnabled()) {
                    this.log.info("*** GET: Giving up on " + DebugLogger.quote(name) + " - retry count has exceeded " + ((Object) DynamicLinker.getLinkedCallSiteLocation()));
                }
                return null;
            }
            MethodHandle cmh = constantGetter(c2);
            if (isOptimistic) {
                if (JSType.getAccessorTypeIndex(cmh.type().returnType()) <= JSType.getAccessorTypeIndex(retType)) {
                    ...
                    mh = Lookup.MH.dropArguments(Lookup.MH.insertArguments(JSType.THROW_UNWARRANTED.methodHandle(), 0, c2, Integer.valueOf(programPoint)), 0, Object.class);
                }
            } else {
            ...
            if (this.log.isEnabled()) {
                this.log.info("Linked getter " + DebugLogger.quote(name) + " as MethodHandle.constant() -> " + c2 + " " + ((Object) acc.getSwitchPoint()));
                mh = MethodHandleFactory.addDebugPrintout(this.log, Level.FINE, mh, "get const " + ((Object) acc));
            }

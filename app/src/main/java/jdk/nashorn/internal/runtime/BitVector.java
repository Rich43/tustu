...
/* loaded from: nashorn.jar:jdk/nashorn/internal/runtime/BitVector.class */
public final class BitVector implements Cloneable {
    ...
    public void set(long bit) {
        long[] jArr = this.bits;
        int i2 = (int) (bit >> 6);
        jArr[i2] = jArr[i2] | (1 << ((int) (bit & 63)));
    }

    public void clear(long bit) {
        long[] jArr = this.bits;
        int i2 = (int) (bit >> 6);
        jArr[i2] = jArr[i2] & ((1 << ((int) (bit & 63))) ^ (-1));
    }
        ...
        if (shift != 0) {
            ...
            if (leftShift == 0) {
                ...
                for (int slot2 = 0; slot2 < slotCount; slot2++) {
                    long jSlot = slot(from2) >>> leftShift;
                    from2++;
                    this.bits[slot2] = jSlot | (slot(from2) << rightShift);
                }
            }
        }
        resize(length);

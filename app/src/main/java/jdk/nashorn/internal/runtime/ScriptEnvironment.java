...
/* loaded from: nashorn.jar:jdk/nashorn/internal/runtime/ScriptEnvironment.class */
public final class ScriptEnvironment {
    ...
    /* loaded from: nashorn.jar:jdk/nashorn/internal/runtime/ScriptEnvironment$FunctionStatementBehavior.class */
    ...
    public ScriptEnvironment(Options options, PrintWriter out, PrintWriter err) {
        ...
        this._dest_dir = options.getString(PdfOps.d_TOKEN);
        ...
        this._fx = options.getBoolean(FXMLLoader.FX_NAMESPACE_PREFIX);
        ...
        this._strict = options.getBoolean(SchemaSymbols.ATTVAL_STRICT);
        ...
        if (pc != null) {
            ...
            while (st.hasMoreTokens()) {
                StringTokenizer st2 = new StringTokenizer(st.nextToken(), CallSiteDescriptor.TOKEN_DELIMITER);
                while (st2.hasMoreTokens()) {
                    String cmd = st2.nextToken();
                    if ("dir".equals(cmd)) {
                        dir = st2.nextToken();
                    } else if (Constants.EXSLT_ELEMNAME_FUNCTION_STRING.equals(cmd)) {
                        func = st2.nextToken();
                    }
                }
            }
        ...
        int callSiteFlags = 0;
        callSiteFlags = options.getBoolean("profile.callsites") ? 0 | 64 : callSiteFlags;
        if (options.get("trace.callsites") instanceof KeyValueOption) {
            callSiteFlags |= 128;
            KeyValueOption kv = (KeyValueOption) options.get("trace.callsites");
            callSiteFlags = kv.hasValue("miss") ? callSiteFlags | 256 : callSiteFlags;
            callSiteFlags = (kv.hasValue("enterexit") || (callSiteFlags & 256) == 0) ? callSiteFlags | 512 : callSiteFlags;
        ...
        this._timing = new Timing((timeLoggerInfo == null || timeLoggerInfo.getLevel() == Level.OFF) ? false : true);
    }

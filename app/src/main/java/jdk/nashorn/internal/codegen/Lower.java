...
@Logger(name = "lower")
/* loaded from: nashorn.jar:jdk/nashorn/internal/codegen/Lower.class */
final class Lower extends NodeOperatorVisitor<BlockLexicalContext> implements Loggable {
    ...
    public static <T extends Node> T ensureUniqueNamesIn(T t2) {
        return (T) t2.accept(new SimpleNodeVisitor() { // from class: jdk.nashorn.internal.codegen.Lower.2
            @Override // jdk.nashorn.internal.ir.visitor.NodeVisitor
            public Node leaveFunctionNode(FunctionNode functionNode) {
    ...
    private TryNode spliceFinally(TryNode tryNode, final ThrowNode rethrow, Block finallyBody) {
        ...
        addStatement(inlinedFinallies.isEmpty() ? newTryNode : newTryNode.setInlinedFinallies(this.lc, inlinedFinallies));
    ...
    public Node leaveTryNode(TryNode tryNode) {
        ...
        return (TryNode) ((BlockLexicalContext) this.lc).replace(tryNode, spliceFinally(newTryNode.setCatchBlocks(this.lc, Arrays.asList(catchAll)), rethrows.get(0), finallyBody));
    }
    ...
    private <T extends LoopNode> T checkEscape(T t2) {
        boolean zControlFlowEscapes = controlFlowEscapes(this.lc, t2.getBody());
        if (zControlFlowEscapes) {
            return (T) t2.setBody(this.lc, t2.getBody().setIsTerminal(this.lc, false)).setControlFlowEscapes(this.lc, zControlFlowEscapes);
        }
        return t2;
    }
    ...
    private void addStatementEnclosedInBlock(Statement stmt) {
        BlockStatement b2 = BlockStatement.createReplacement(stmt, Collections.singletonList(stmt));
        if (stmt.isTerminal()) {
            b2 = b2.setBlock(b2.getBlock().setIsTerminal(null, true));
        }
        addStatement(b2);
    }

...
@Logger(name = "fields")
/* loaded from: nashorn.jar:jdk/nashorn/internal/codegen/ObjectClassGenerator.class */
public final class ObjectClassGenerator implements Loggable {
    ...
    public static long pack(Number n2) {
        ...
        if (n2 instanceof Double) {
            return Double.doubleToRawLongBits(n2.doubleValue());
        }
        throw new AssertionError((Object) ("cannot pack" + ((Object) n2)));
    }
    ...
    private static MethodEmitter newInitWithSpillArraysMethod(ClassEmitter classEmitter, Class<?> superClass) {
        ...
        init.invoke(CompilerConstants.constructorNoLookup(superClass, (Class<?>[]) new Class[]{PropertyMap.class, long[].class, Object[].class}));
        return init;
    }
    ...
    private static void newEmptyInit(String className, ClassEmitter classEmitter) {
        ...
        emptyInit.loadNull();
        emptyInit.invoke(CompilerConstants.constructorNoLookup(className, (Class<?>[]) new Class[]{PropertyMap.class}));
    ...
    private static void newAllocate(String className, ClassEmitter classEmitter) {
        ...
        allocate.begin();
        allocate._new(className, Type.typeFor((Class<?>) ScriptObject.class));
        allocate.dup();
        allocate.load(Type.typeFor((Class<?>) PropertyMap.class), 0);
        allocate.invoke(CompilerConstants.constructorNoLookup(className, (Class<?>[]) new Class[]{PropertyMap.class}));
        allocate._return();
        allocate.end();
    }

    private byte[] toByteArray(String className, ClassEmitter classEmitter) throws IOException {
        classEmitter.end();
    ...
    private static Object getDifferent(Object receiver, Class<?> forType, MethodHandle primitiveGetter, MethodHandle objectGetter, int programPoint) {
        ...
        try {
            Object value = (Object) mh.invokeExact(receiver);
    ...
    private static MethodHandle createGetterInner(Class<?> forType, Class<?> type, MethodHandle primitiveGetter, MethodHandle objectGetter, List<MethodHandle> converters, int programPoint) {
        ...
        MethodHandle methodHandle = (primitiveGetter != null && isPrimitiveStorage) ? primitiveGetter : objectGetter;
        MethodHandle getter = methodHandle;
        if (forType == null) {
            return isOptimistic ? ti == 2 ? Lookup.MH.dropArguments(JSType.GET_UNDEFINED.get(2), 0, Object.class) : Lookup.MH.asType(Lookup.MH.dropArguments(Lookup.MH.insertArguments(GET_DIFFERENT_UNDEFINED, 0, Integer.valueOf(programPoint)), 0, Object.class), getter.type().changeReturnType(type)) : Lookup.MH.dropArguments(JSType.GET_UNDEFINED.get(ti), 0, Object.class);
        }
        if (!$assertionsDisabled && primitiveGetter == null && forType != Object.class) {
            throw new AssertionError(forType);
        }
        if (isOptimistic) {
            if (fti < ti) {
            ...
            if (fti == 2) {
                return Lookup.MH.filterReturnValue(objectGetter, Lookup.MH.insertArguments(converters.get(ti), 1, Integer.valueOf(programPoint)));
            }
            return Lookup.MH.asType(Lookup.MH.filterArguments(objectGetter, 0, Lookup.MH.insertArguments(GET_DIFFERENT, 1, forType, primitiveGetter, objectGetter, Integer.valueOf(programPoint))), objectGetter.type().changeReturnType(type));
        }
        if (!$assertionsDisabled && isOptimistic) {
            throw new AssertionError();
        }
        MethodHandle tgetter2 = getterForType(forType, primitiveGetter, objectGetter);
        if (fti == 2) {
            if (fti != ti) {
                return Lookup.MH.filterReturnValue(tgetter2, JSType.CONVERT_OBJECT.get(ti));
            }
            return tgetter2;
        }
        if (!$assertionsDisabled && primitiveGetter == null) {
            throw new AssertionError();
        }
        MethodType tgetterType = tgetter2.type();
        switch (fti) {
            case 0:
                return Lookup.MH.asType(tgetter2, tgetterType.changeReturnType(type));
            case 1:
                switch (ti) {
                    case 0:
                        return Lookup.MH.filterReturnValue(tgetter2, JSType.TO_INT32_D.methodHandle);
                    case 1:
                        ...
                        throw new AssertionError();
                    default:
                        return Lookup.MH.asType(tgetter2, tgetterType.changeReturnType(Object.class));
                }
            default:
                throw new UnsupportedOperationException(((Object) forType) + "=>" + ((Object) type));
        }
    }
    ...
    public static MethodHandle createSetter(Class<?> forType, Class<?> type, MethodHandle primitiveSetter, MethodHandle objectSetter) {
        ...
        switch (fti) {
            ...
            case 1:
                return ti == 2 ? objectSetter : Lookup.MH.asType(Lookup.MH.filterArguments(primitiveSetter, 1, PACK_DOUBLE), pmt.changeParameterType(1, type));
            default:
                throw new UnsupportedOperationException(((Object) forType) + "=>" + ((Object) type));
        }
    }

    private static boolean isType(Class<?> boxedForType, Object x2) {
        return x2 != null && x2.getClass() == boxedForType;
    }

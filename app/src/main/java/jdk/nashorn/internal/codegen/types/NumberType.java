...
/* loaded from: nashorn.jar:jdk/nashorn/internal/codegen/types/NumberType.class */
class NumberType extends NumericType {
    ...
    static {
        ...
        VALUE_OF = CompilerConstants.staticCallNoLookup(Double.class, BeanAdapter.VALUE_OF_METHOD_NAME, Double.class, Double.TYPE);
    }

    protected NumberType() {
        super(SchemaSymbols.ATTVAL_DOUBLE, Double.TYPE, 4, 2);
    }
    ...
    public Type ldc(MethodVisitor method, Object c2) {
        if (!$assertionsDisabled && !(c2 instanceof Double)) {
            throw new AssertionError();
        }
        double value = ((Double) c2).doubleValue();

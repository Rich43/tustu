...
/* loaded from: nashorn.jar:jdk/nashorn/internal/objects/NativeJSON.class */
public final class NativeJSON extends ScriptObject {
    ...
    /* loaded from: nashorn.jar:jdk/nashorn/internal/objects/NativeJSON$Constructor.class */
    ...
    public static Object stringify(Object self, Object value, Object replacer, Object space) {
        ...
        if (Bootstrap.isCallable(replacer)) {
            ...
            while (iter.hasNext()) {
                ...
                if (v2 instanceof String) {
                    item = (String) v2;
                } else if (v2 instanceof ConsString) {
                    item = v2.toString();
                } else if ((v2 instanceof Number) || (v2 instanceof NativeNumber) || (v2 instanceof NativeString)) {
                    item = JSType.toString(v2);
                }
                if (item != null) {
        ...
        if (modSpace instanceof NativeNumber) {
            modSpace = Double.valueOf(JSType.toNumber(JSType.toPrimitive(modSpace, (Class<?>) Number.class)));
        } else if (modSpace instanceof NativeString) {
            modSpace = JSType.toString(JSType.toPrimitive(modSpace, (Class<?>) String.class));
        }
    ...
    /* loaded from: nashorn.jar:jdk/nashorn/internal/objects/NativeJSON$StringifyState.class */
    ...
    private static Object str(Object key, Object holder, StringifyState state) {
        ...
        try {
            if (value instanceof ScriptObject) {
                ...
                Object toJSON = (Object) toJSONInvoker.getGetter().invokeExact(svalue);
                if (Bootstrap.isCallable(toJSON)) {
                    value = (Object) toJSONInvoker.getInvoker().invokeExact(toJSON, svalue, key);
                }
            } else if (value instanceof JSObject) {
                JSObject jsObj = (JSObject) value;
                Object toJSON2 = jsObj.getMember("toJSON");
                if (Bootstrap.isCallable(toJSON2)) {
                    value = (Object) getJSOBJECT_INVOKER().invokeExact(toJSON2, value);
                }
            }
            if (state.replacerFunction != null) {
                value = (Object) getREPLACER_INVOKER().invokeExact(state.replacerFunction, holder, key, value);
            }
            ...
            if (value instanceof Number) {
                return JSType.isFinite(((Number) value).doubleValue()) ? JSType.toString(value) : FXMLLoader.NULL_KEYWORD;
            }
    ...
    private static String JO(Object value, StringifyState state) {
        ...
        List<String> k2 = state.propertyList == null ? Arrays.asList(getOwnKeys(value)) : state.propertyList;
        for (Object p2 : k2) {
            Object strP = str(p2, value, state);
            if (strP != ScriptRuntime.UNDEFINED) {
                StringBuilder member = new StringBuilder();
                member.append(JSONFunctions.quote(p2.toString())).append(':');

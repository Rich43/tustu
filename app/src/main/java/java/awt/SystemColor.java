...
/* loaded from: rt.jar:java/awt/SystemColor.class */
public final class SystemColor extends Color implements Serializable {
    ...
    private static void updateSystemColors() throws HeadlessException {
        if (!GraphicsEnvironment.isHeadless()) {
            Toolkit.getDefaultToolkit().loadSystemColors(systemColors);
        }
        for (int i2 = 0; i2 < systemColors.length; i2++) {
            systemColorObjects[i2].value = systemColors[i2];
        }
    }

    private SystemColor(byte b2) {
        super(systemColors[b2]);
        this.index = b2;
    }

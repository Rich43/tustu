...
/* loaded from: rt.jar:java/util/concurrent/locks/AbstractQueuedLongSynchronizer.class */
public abstract class AbstractQueuedLongSynchronizer extends AbstractOwnableSynchronizer implements Serializable {
    ...
    /* loaded from: rt.jar:java/util/concurrent/locks/AbstractQueuedLongSynchronizer$Node.class */
    ...
    protected final void setState(long j2) {
        this.state = j2;
    }

    protected final boolean compareAndSetState(long j2, long j3) {
        return unsafe.compareAndSwapLong(this, stateOffset, j2, j3);
    }
    ...
    private void setHeadAndPropagate(Node node, long j2) {
        Node node2;
        Node node3 = this.head;
        setHead(node);
        if (j2 > 0 || node3 == null || node3.waitStatus < 0 || (node2 = this.head) == null || node2.waitStatus < 0) {
    ...
    final boolean acquireQueued(Node node, long j2) {
        ...
        while (true) {
            try {
                ...
                if (nodePredecessor == this.head && tryAcquire(j2)) {
                    break;
                }
    ...
    private void doAcquireInterruptibly(long j2) throws InterruptedException {
    ...
    private boolean doAcquireNanos(long j2, long j3) throws InterruptedException {
        if (j3 <= 0) {
            return false;
        }
    ...
    private void doAcquireShared(long j2) {
        Node nodePredecessor;
        ...
        while (true) {
            try {
                ...
                if (nodePredecessor == this.head) {
                    jTryAcquireShared = tryAcquireShared(j2);
    ...
    private void doAcquireSharedInterruptibly(long j2) throws InterruptedException {
        Node nodeAddWaiter = addWaiter(Node.SHARED);
        while (true) {
            try {
                Node nodePredecessor = nodeAddWaiter.predecessor();
                if (nodePredecessor == this.head) {
                    long jTryAcquireShared = tryAcquireShared(j2);
                    if (jTryAcquireShared >= 0) {
                        setHeadAndPropagate(nodeAddWaiter, jTryAcquireShared);
                        nodePredecessor.next = null;
                        if (0 == 0) {
    ...
    private boolean doAcquireSharedNanos(long j2, long j3) throws InterruptedException {
        if (j3 <= 0) {
            return false;
        }
        long jNanoTime = System.nanoTime() + j3;
    ...
    protected boolean tryAcquire(long j2) {
        throw new UnsupportedOperationException();
    }

    protected boolean tryRelease(long j2) {
        throw new UnsupportedOperationException();
    }
    ...
    public final void acquire(long j2) {
        if (!tryAcquire(j2) && acquireQueued(addWaiter(Node.EXCLUSIVE), j2)) {
            selfInterrupt();
        }
    }

    public final void acquireInterruptibly(long j2) throws InterruptedException {
        if (Thread.interrupted()) {
            throw new InterruptedException();
        }
        if (!tryAcquire(j2)) {
            doAcquireInterruptibly(j2);
        }
    }

    public final boolean tryAcquireNanos(long j2, long j3) throws InterruptedException {
        if (Thread.interrupted()) {
            throw new InterruptedException();
        }
        return tryAcquire(j2) || doAcquireNanos(j2, j3);
    }

    public final boolean release(long j2) {
        if (tryRelease(j2)) {
            Node node = this.head;
    ...
    public final void acquireShared(long j2) {
        if (tryAcquireShared(j2) < 0) {
            doAcquireShared(j2);
        }
    }

    public final void acquireSharedInterruptibly(long j2) throws InterruptedException {
        if (Thread.interrupted()) {
            throw new InterruptedException();
        }
        if (tryAcquireShared(j2) < 0) {
            doAcquireSharedInterruptibly(j2);
        }
    }

    public final boolean tryAcquireSharedNanos(long j2, long j3) throws InterruptedException {
        if (Thread.interrupted()) {
            throw new InterruptedException();
        }
        return tryAcquireShared(j2) >= 0 || doAcquireSharedNanos(j2, j3);
    }

    public final boolean releaseShared(long j2) {
        if (tryReleaseShared(j2)) {
            doReleaseShared();
            return true;
        }
        return false;
    ...
    */
    ...
    /* loaded from: rt.jar:java/util/concurrent/locks/AbstractQueuedLongSynchronizer$ConditionObject.class */
    public class ConditionObject implements Condition, Serializable {
        ...
        public final void awaitUninterruptibly() {
            ...
            if (AbstractQueuedLongSynchronizer.this.acquireQueued(nodeAddConditionWaiter, jFullyRelease) || z2) {
        ...
        public final void await() throws InterruptedException {
            ...
            if (AbstractQueuedLongSynchronizer.this.acquireQueued(nodeAddConditionWaiter, jFullyRelease) && i2 != -1) {
                i2 = 1;
            }

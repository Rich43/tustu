...
/* loaded from: rt.jar:java/time/ZoneId.class */
public abstract class ZoneId implements Serializable {
    ...
    static ZoneId of(String str, boolean z2) {
        Objects.requireNonNull(str, "zoneId");
        if (str.length() <= 1 || str.startsWith(Marker.ANY_NON_NULL_MARKER) || str.startsWith(LanguageTag.SEP)) {
            return ZoneOffset.of(str);
        }
        if (str.startsWith("UTC") || str.startsWith("GMT")) {
            return ofWithPrefix(str, 3, z2);
        }
        if (str.startsWith("UT")) {
            return ofWithPrefix(str, 2, z2);
        }
        return ZoneRegion.ofId(str, z2);
    }

    private static ZoneId ofWithPrefix(String str, int i2, boolean z2) {
        ...
        try {
            ZoneOffset zoneOffsetOf = ZoneOffset.of(str.substring(i2));
